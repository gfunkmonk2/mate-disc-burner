...
typedef struct _RejillaJacketBackgroundPrivate RejillaJacketBackgroundPrivate;
struct _RejillaJacketBackgroundPrivate
{
	gchar *path;
...
#define REJILLA_JACKET_BACKGROUND_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), REJILLA_TYPE_JACKET_BACKGROUND, RejillaJacketBackgroundPrivate))



G_DEFINE_TYPE (RejillaJacketBackground, rejilla_jacket_background, GTK_TYPE_DIALOG);

RejillaJacketColorStyle
rejilla_jacket_background_get_color_style (RejillaJacketBackground *self)
{
	RejillaJacketBackgroundPrivate *priv;

	priv = REJILLA_JACKET_BACKGROUND_PRIVATE (self);
	return gtk_combo_box_get_active (GTK_COMBO_BOX (priv->color_style));
}

RejillaJacketImageStyle
rejilla_jacket_background_get_image_style (RejillaJacketBackground *self)
{
	RejillaJacketBackgroundPrivate *priv;
...
gchar *
rejilla_jacket_background_get_image_path (RejillaJacketBackground *self)
{
	RejillaJacketBackgroundPrivate *priv;
...
void
rejilla_jacket_background_set_color_style (RejillaJacketBackground *self,
...
void
rejilla_jacket_background_get_color (RejillaJacketBackground *self,
				     GdkColor *color,
				     GdkColor *color2)
{
	RejillaJacketBackgroundPrivate *priv;
...
void
rejilla_jacket_background_set_image_style (RejillaJacketBackground *self,
...
void
rejilla_jacket_background_set_image_path (RejillaJacketBackground *self,
					  const gchar *path)
{
	RejillaJacketBackgroundPrivate *priv;
...
void
rejilla_jacket_background_set_color (RejillaJacketBackground *self,
				     GdkColor *color,
				     GdkColor *color2)
{
	RejillaJacketBackgroundPrivate *priv;
...
static void
rejilla_jacket_background_color_type_changed_cb (GtkComboBox *combo,
						 RejillaJacketBackground *self)
{
	RejillaJacketBackgroundPrivate *priv;

	priv = REJILLA_JACKET_BACKGROUND_PRIVATE (self);

	if (gtk_combo_box_get_active (combo) == REJILLA_JACKET_COLOR_SOLID) {
...
static void
rejilla_jacket_background_add_filters (RejillaJacketBackground *self)
{
	RejillaJacketBackgroundPrivate *priv;
...
static void
rejilla_jacket_background_init (RejillaJacketBackground *object)
{
	RejillaJacketBackgroundPrivate *priv;
	GtkWidget *table;
	GtkWidget *combo;
	...
	priv = REJILLA_JACKET_BACKGROUND_PRIVATE (object);
	...
	g_signal_connect (combo,
			  "changed",
			  G_CALLBACK (rejilla_jacket_background_color_type_changed_cb),
			  object);
	...
	rejilla_jacket_background_add_filters (object);
	gtk_window_set_default_size (GTK_WINDOW (object), 400, 240);
}

static void
rejilla_jacket_background_finalize (GObject *object)
{
	G_OBJECT_CLASS (rejilla_jacket_background_parent_class)->finalize (object);
}

static void
rejilla_jacket_background_class_init (RejillaJacketBackgroundClass *klass)
{
	GObjectClass* object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (RejillaJacketBackgroundPrivate));

	object_class->finalize = rejilla_jacket_background_finalize;
}

GtkWidget *
rejilla_jacket_background_new (void)
{
	return g_object_new (REJILLA_TYPE_JACKET_BACKGROUND,
