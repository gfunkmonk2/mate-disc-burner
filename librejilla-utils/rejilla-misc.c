...
GQuark
rejilla_utils_error_quark (void)
{
	static GQuark quark = 0;

	if (!quark)
		quark = g_quark_from_static_string ("Rejilla_utils_error");
...
static gboolean
rejilla_utils_clear_strings_cb (gchar *string,
				guint ref,
				gpointer NULL_data)
{
	g_free (string);
...
static void
rejilla_utils_free (void)
{
	if (stringsH) {
		G_LOCK (stringsH);
		g_hash_table_foreach_remove (stringsH,
					     (GHRFunc) rejilla_utils_clear_strings_cb,
					     NULL);
...
rejilla_utils_get_uri_name (const gchar *uri)
{
	...
	utf8_name = rejilla_utils_validate_utf8 (name);
	if (utf8_name) {
		g_free (name);
		return utf8_name;
	}
...
GtkWidget *
rejilla_utils_properties_get_label (GtkWidget *properties)
{
	GList *children;
	GList *iter;
...
rejilla_utils_pack_properties (const gchar *title, ...)
{
	...
	result = rejilla_utils_pack_properties_list (title, list);
	g_slist_free (list);
				     ...
				     GtkMessageType type)
{
	...
	gtk_window_set_icon_name (GTK_WINDOW (message),
	                          parent? gtk_window_get_icon_name (GTK_WINDOW (parent)):"rejilla");
			      ...
			      GtkMessageType type)
{
	...
	message = rejilla_utils_create_message_dialog (parent,
						       primary_message,
						       secondary_message,
						       type);
