...
#define REJILLA_METADATA_SILENCE_INTERVAL		100000000LL
#define REJILLA_METADATA_INITIAL_STATE			GST_STATE_PAUSED

struct RejillaMetadataPrivate {
	GstElement *pipeline;
	GstElement *source;
	GstElement *decode;
	GstElement *convert;
	...
	RejillaMetadataSilence *silence;
	...
	RejillaMetadataGetXidCb xid_callback;
...
typedef struct RejillaMetadataPrivate RejillaMetadataPrivate;
#define REJILLA_METADATA_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), REJILLA_TYPE_METADATA, RejillaMetadataPrivate))
...
typedef enum {
	COMPLETED_SIGNAL,
	LAST_SIGNAL
} RejillaMetadataSignalType;

static guint rejilla_metadata_signals [LAST_SIGNAL] = { 0 };

#define REJILLA_METADATA_IS_FAST(flags) 					\
	(!((flags) & REJILLA_METADATA_FLAG_SILENCES) &&				\
	((flags) & REJILLA_METADATA_FLAG_FAST))

G_DEFINE_TYPE (RejillaMetadata, rejilla_metadata, G_TYPE_OBJECT)

static GSList *downloading = NULL;
static GSList *downloaded = NULL;

static gboolean
rejilla_metadata_completed (RejillaMetadata *self);

static int
rejilla_metadata_get_xid (RejillaMetadata *metadata)
{
	RejillaMetadataPrivate *priv;

	priv = REJILLA_METADATA_PRIVATE (metadata);
...
struct _RejillaMetadataGstDownload {
	gchar *detail;

	/* These are all metadata objects waiting */
	GSList *objects;
};
typedef struct _RejillaMetadataGstDownload RejillaMetadataGstDownload;
...
rejilla_metadata_info_free (RejillaMetadataInfo *info)
{
	...
	rejilla_metadata_info_clear (info);
			    ...
			    RejillaMetadataInfo *src)
{
	...
	for (iter = src->silences; iter; iter = iter->next) {
		RejillaMetadataSilence *silence, *copy;

		silence = iter->data;

		copy = g_new0 (RejillaMetadataSilence, 1);
...
static void
rejilla_metadata_stop_pipeline (GstElement *pipeline)
{
	GstState state;
	...
	while (change == GST_STATE_CHANGE_ASYNC && state != GST_STATE_NULL) {
		...
		change = gst_element_get_state (pipeline,
						...
						&pending,
						GST_MSECOND);
		REJILLA_UTILS_LOG ("Get state (current = %i pending = %i) returned %i",
...
static void
rejilla_metadata_destroy_pipeline (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;

	priv = REJILLA_METADATA_PRIVATE (self);

	priv->started = 0;

	if (priv->pipeline_mp3) {
		rejilla_metadata_stop_pipeline (priv->pipeline_mp3);
	...
	rejilla_metadata_stop_pipeline (priv->pipeline);
...
static void
rejilla_metadata_stop (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
	GSList *iter;
	...
	REJILLA_UTILS_LOG ("Retrieval ended for %s %p",
	...
	if (priv->pipeline)
		rejilla_metadata_destroy_pipeline (self);
	...
	if (priv->downloads) {
		...
		for (iter = priv->downloads; iter; iter = iter->next) {
			RejillaMetadataGstDownload *download;
...
void
rejilla_metadata_cancel (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;

	priv = REJILLA_METADATA_PRIVATE (self);

	REJILLA_UTILS_LOG ("Metadata retrieval cancelled for %s %p",
			   priv->info ? priv->info->uri:"Unknown",
			   self);
	
	rejilla_metadata_stop (self);
	if (priv->error) {
...
static void
rejilla_metadata_install_plugins_add_downloaded (GSList *downloads)
{
	GSList *iter;
...
static void
rejilla_metadata_install_plugins_free_data (GSList *downloads)
{
	GSList *iter;
	...
	for (iter = downloads; iter; iter = iter->next) {
		...
		for (meta = download->objects; meta; meta = meta->next) {
			...
			priv = REJILLA_METADATA_PRIVATE (meta->data);
...
static void
rejilla_metadata_install_plugins_success (RejillaMetadataGstDownload *download)
{
	GSList *iter;
	...
	for (iter = download->objects; iter; iter = iter->next) {
		...
		priv = REJILLA_METADATA_PRIVATE (iter->data);
...
static void
rejilla_metadata_install_plugins_abort (RejillaMetadataGstDownload *download)
{
	GSList *iter;
	GSList *next;
	...
	for (iter = download->objects; iter; iter = next) {
		...
		rejilla_metadata_completed (REJILLA_METADATA (iter->data));
	}
}

static void
rejilla_metadata_install_plugins_completed (RejillaMetadataGstDownload *download)
{
	GSList *iter;
	GSList *next;
...
static void
rejilla_metadata_install_plugins_result (GstInstallPluginsReturn res,
					 gpointer data)
{
	...
	case GST_INSTALL_PLUGINS_SUCCESS:
		rejilla_metadata_install_plugins_add_downloaded (downloads);
		...
		for (iter = downloads; iter; iter = iter->next) {
			...
			download = iter->data;
			rejilla_metadata_install_plugins_success (download);
		}
		break;
	...
	case GST_INSTALL_PLUGINS_NOT_FOUND:
		...
		/* stop everything */
		for (iter = downloads; iter; iter = iter->next)
			rejilla_metadata_install_plugins_completed (iter->data);
		break;
	...
	case GST_INSTALL_PLUGINS_USER_ABORT:
		...
		for (iter = downloads; iter; iter = iter->next) {
			...
			download = iter->data;
			rejilla_metadata_install_plugins_abort (download);
		}
		break;
	...
	rejilla_metadata_install_plugins_free_data (downloads);
}

static RejillaMetadataGstDownload *
rejilla_metadata_is_downloading (const gchar *detail)
{
	GSList *iter;
...
static gboolean
rejilla_metadata_install_missing_plugins (RejillaMetadata *self)
{
	...
	REJILLA_UTILS_LOG ("Starting to download missing plugins");
	...
	for (iter = priv->missing_plugins; iter; iter = iter->next) {
		...
		download = rejilla_metadata_is_downloading (detail);
		if (download) {
		...
		download = g_new0 (RejillaMetadataGstDownload, 1);
	...
	gst_install_plugins_context_set_xid (context, rejilla_metadata_get_xid (self));
	status = gst_install_plugins_async ((gchar **) details->pdata,
					    context,
					    rejilla_metadata_install_plugins_result,
					    downloads);

	gst_install_plugins_context_free (context);
	g_ptr_array_free (details, TRUE);

	REJILLA_UTILS_LOG ("Download status %i", status);
...
rejilla_metadata_completed (RejillaMetadata *self)
{
	...
	if (priv->error) {
		REJILLA_UTILS_LOG ("Operation completed with an error %s", priv->error->message);
	}

	/* See if we have missing plugins */
	if (priv->missing_plugins) {
		if (rejilla_metadata_install_missing_plugins (self))
			return TRUE;
	}
	...
	g_signal_emit (G_OBJECT (self),
		       rejilla_metadata_signals [COMPLETED_SIGNAL],
		       0,
		       priv->error);
...
static gboolean
rejilla_metadata_thumbnail (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
	gint64 position;
	gboolean res;
	...
	REJILLA_UTILS_LOG ("Seeking forward %i for %s", res, priv->info->uri);
	if (!res)
		return rejilla_metadata_completed (self);
...
static void
rejilla_metadata_is_seekable (RejillaMetadata *self)
{
	GstQuery *query;
	GstFormat format;
	gboolean seekable;
...
static gboolean
rejilla_metadata_get_mime_type (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
	GstElement *typefind;
	GstCaps *caps = NULL;
	...
	REJILLA_UTILS_LOG ("Mime type %s", mime);
	...
	if (!strcmp (mime, "application/x-id3"))
		...
		REJILLA_UTILS_LOG ("Wav file has dts: %s", priv->info->has_dts? "yes":"no");
	}
	else
		priv->info->type = g_strdup (mime);
...
static gboolean
rejilla_metadata_is_mp3 (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
	...
	if (!priv->info->type
	&&  !rejilla_metadata_get_mime_type (self))
		return FALSE;
...
static gboolean
rejilla_metadata_process_element_messages (RejillaMetadata *self,
					   GstMessage *msg)
{
	RejillaMetadataPrivate *priv;
	...
	||  !strcmp (gst_structure_get_name (msg->structure), "pixbuf")) {
		...
		REJILLA_UTILS_LOG ("Received pixbuf snapshot sink (%p) for %s", priv->info->snapshot, priv->info->uri);
	...
	if ((priv->flags & REJILLA_METADATA_FLAG_MISSING)
	...
	&&   gst_structure_has_field (msg->structure, "peak")) {
		...
		if (peak < -50.0) {
			...
			if (pos == -1) {
				REJILLA_UTILS_LOG ("impossible to retrieve position");
				return TRUE;
			}

			if (!priv->silence) {
				priv->silence = g_new0 (RejillaMetadataSilence, 1);
			...
			REJILLA_UTILS_LOG ("silence detected at %lli", pos);
		}
		else if (priv->silence) {
			REJILLA_UTILS_LOG ("silence finished");
...
static void
rejilla_metadata_process_pending_messages (RejillaMetadata *self)
{
	GstBus *bus;
	GstMessage *msg;
	RejillaMetadataPrivate *priv;
	...
	while ((msg = gst_bus_pop (bus))) {
		...
		if (GST_MESSAGE_TYPE (msg) == GST_MESSAGE_TAG) {
			...
			rejilla_metadata_process_element_messages (self, msg);
...
static gboolean
rejilla_metadata_success (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;

	priv = REJILLA_METADATA_PRIVATE (self);

	REJILLA_UTILS_LOG ("Metadata retrieval completed for %s", priv->info->uri);

	/* check if that's a seekable one */
	rejilla_metadata_is_seekable (self);
	...
	&& !priv->snapshot_started)
		return rejilla_metadata_thumbnail (self);

	return rejilla_metadata_completed (self);
}

static gboolean
rejilla_metadata_get_duration (RejillaMetadata *self,
			       GstElement *pipeline,
			       gboolean use_duration)
{
	...
	if (duration == -1) {
		if (!priv->error) {
			...
			REJILLA_GET_BASENAME_FOR_DISPLAY (priv->info->uri, name);
			priv->error = g_error_new (REJILLA_UTILS_ERROR,
	...
	REJILLA_UTILS_LOG ("Found duration %lli for %s", duration, priv->info->uri);

	priv->info->len = duration;
	return rejilla_metadata_success (self);
}
...
static gboolean
rejilla_metadata_mp3_bus_messages (GstBus *bus,
				   GstMessage *msg,
				   RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
	...
	case GST_MESSAGE_ERROR:
		...
		REJILLA_UTILS_LOG ("GStreamer error - mp3 - (%s)", debug_string);
		g_free (debug_string);
		if (!priv->error && error)
			priv->error = error;

		rejilla_metadata_completed (self);
		return FALSE;

	case GST_MESSAGE_EOS:
		REJILLA_UTILS_LOG ("End of stream reached - mp3 - for %s", priv->info->uri);
		rejilla_metadata_get_duration (self, priv->pipeline_mp3, FALSE);
		return FALSE;
...
static gboolean
rejilla_metadata_create_mp3_pipeline (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
	GstElement *source;
	GstElement *parse;
	...
	priv->watch_mp3 = gst_bus_add_watch (bus,
					     (GstBusFunc) rejilla_metadata_mp3_bus_messages,
					     self);
...
static gboolean
rejilla_metadata_success_main (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;

	priv = REJILLA_METADATA_PRIVATE (self);

	REJILLA_UTILS_LOG ("Metadata retrieval successfully completed for %s", priv->info->uri);

	/* find the type of the file */
	rejilla_metadata_get_mime_type (self);

	/* empty the bus of any pending message */
	rejilla_metadata_process_pending_messages (self);

	/* get the size */
	if (rejilla_metadata_is_mp3 (self)) {
		if (!rejilla_metadata_create_mp3_pipeline (self)) {
			REJILLA_UTILS_LOG ("Impossible to run mp3 specific pipeline");
			return rejilla_metadata_completed (self);
		}
	...
	return rejilla_metadata_get_duration (self, priv->pipeline, TRUE);
}

static gboolean
rejilla_metadata_bus_messages (GstBus *bus,
			       GstMessage *msg,
			       RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
	...
	case GST_MESSAGE_ELEMENT:
		return rejilla_metadata_process_element_messages (self, msg);

	case GST_MESSAGE_ERROR:
		/* save the error message */
		gst_message_parse_error (msg, &error, &debug_string);
		REJILLA_UTILS_LOG ("GStreamer error (%s)", debug_string);
		g_free (debug_string);
		if (!priv->error && error)
			priv->error = error;
	...
	case GST_MESSAGE_EOS:
		REJILLA_UTILS_LOG ("End of stream reached for %s", priv->info->uri);
		return rejilla_metadata_success_main (self);
...
static gboolean
rejilla_metadata_create_audio_pipeline (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
	GstPad *audio_pad;
	...
	if (priv->flags & REJILLA_METADATA_FLAG_SILENCES) {
		...
		if (!priv->level) {
			...
			g_object_set (priv->level,
				      "message", TRUE,
				      "interval", (guint64) REJILLA_METADATA_SILENCE_INTERVAL,
				      NULL);
		}
		...
		if (!gst_element_link_many (priv->convert,
		                            ...
		                            priv->sink,
		                            NULL)) {
			REJILLA_UTILS_LOG ("Impossible to link elements");
			gst_object_unref (priv->audio);
			priv->audio = NULL;
			return FALSE;
		}
	...
	else if (priv->flags & REJILLA_METADATA_FLAG_THUMBNAIL) {
		GstElement *queue;
	...
	REJILLA_UTILS_LOG ("Adding audio pipeline for %s", priv->info->uri);

	return TRUE;
}

static gboolean
rejilla_metadata_create_video_pipeline (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
	...
	if (!priv->snapshot) {
		...
		REJILLA_UTILS_LOG ("gdkpixbufsink is not installed");
		return FALSE;
	}
	...
	if (!colorspace) {
		...
		REJILLA_UTILS_LOG ("ffmpegcolorspace is not installed");
		return FALSE;
	}
	...
	REJILLA_UTILS_LOG ("Adding pixbuf snapshot sink for %s", priv->info->uri);

	return TRUE;
}

static void
rejilla_metadata_error_on_pad_linking (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
	GstMessage *message;
	GstBus *bus;
	...
	message = gst_message_new_error (GST_OBJECT (priv->pipeline),
					 priv->error,
					 "Sent by rejilla_metadata_error_on_pad_linking");
...
static gboolean
rejilla_metadata_link_dummy_pad (RejillaMetadata *self,
				 GstPad *pad)
{
	RejillaMetadataPrivate *priv;
	GstElement *fakesink;
	...
	REJILLA_UTILS_LOG ("Linking to a fake sink");
	...
	if (res == GST_PAD_LINK_OK) {
		gst_element_set_state (fakesink, REJILLA_METADATA_INITIAL_STATE);
		return TRUE;
	}
...
static void
rejilla_metadata_audio_caps (RejillaMetadata *self,
                             GstCaps *caps)
{
	int i;
	int num_caps;
	...
	for (i = 0; i < num_caps; i++) {
		...
		if (gst_structure_has_field (structure, "channels")) {
			if (gst_structure_get_field_type (structure, "channels") == G_TYPE_INT) {
				...
				REJILLA_UTILS_LOG ("Number of channels %i", priv->info->channels);
			}
				...
				REJILLA_UTILS_LOG ("Unhandled type for channel prop %s",
		...
		if (gst_structure_has_field (structure, "rate")) {
			if (gst_structure_get_field_type (structure, "rate") == G_TYPE_INT) {
				...
				REJILLA_UTILS_LOG ("Rate %i", priv->info->rate);
			}
				...
				REJILLA_UTILS_LOG ("Unhandled type for rate prop %s",
...
static void
rejilla_metadata_new_decoded_pad_cb (GstElement *decode,
				     GstPad *pad,
				     ...
				     RejillaMetadata *self)
{
	...
	REJILLA_UTILS_LOG ("New pad for %s", priv->info->uri);
	...
	if (has_audio && !priv->audio_linked) {
		rejilla_metadata_audio_caps (self, caps);
		rejilla_metadata_create_audio_pipeline (self);
		sink = gst_element_get_static_pad (priv->audio, "sink");
		if (sink && !GST_PAD_IS_LINKED (sink)) {
			res = gst_pad_link (pad, sink);
			REJILLA_UTILS_LOG ("Audio stream link %i for %s", res, priv->info->uri);
			gst_object_unref (sink);

			priv->audio_linked = (res == GST_PAD_LINK_OK);
			gst_element_set_state (priv->audio, REJILLA_METADATA_INITIAL_STATE);
		}
	}

	if (g_strrstr (name, "video/x-raw-") && !priv->video_linked) {
		REJILLA_UTILS_LOG ("RAW video stream found");

		if (!priv->video && (priv->flags & REJILLA_METADATA_FLAG_THUMBNAIL)) {
			...
			if (!rejilla_metadata_create_video_pipeline (self)) {
				REJILLA_UTILS_LOG ("Impossible to create video pipeline");

				gst_caps_unref (caps);

				if (!rejilla_metadata_link_dummy_pad (self, pad))
					rejilla_metadata_error_on_pad_linking (self);
			...
			gst_element_set_state (priv->video, REJILLA_METADATA_INITIAL_STATE);

			REJILLA_UTILS_LOG ("Video stream link %i for %s", res, priv->info->uri);
		}
		else if (!rejilla_metadata_link_dummy_pad (self, pad))
			rejilla_metadata_error_on_pad_linking (self);
	}
	else if (has_video && !rejilla_metadata_link_dummy_pad (self, pad))
		rejilla_metadata_error_on_pad_linking (self);
...
static gboolean
rejilla_metadata_create_pipeline (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
	...
	g_signal_connect (G_OBJECT (priv->decode), "new-decoded-pad",
			  G_CALLBACK (rejilla_metadata_new_decoded_pad_cb),
			  self);
...
static gboolean
rejilla_metadata_set_new_uri (RejillaMetadata *self,
			      const gchar *uri)
{
	RejillaMetadataPrivate *priv;
	GstBus *bus;
	...
	REJILLA_UTILS_LOG ("New retrieval for %s %p", uri, self);
	...
	rejilla_metadata_info_free (priv->info);
	priv->info = NULL;
	...
	priv->info = g_new0 (RejillaMetadataInfo, 1);
	...
	else if (!rejilla_metadata_create_pipeline (self))
		return FALSE;
	...
	priv->watch = gst_bus_add_watch (bus,
					 (GstBusFunc) rejilla_metadata_bus_messages,
					 self);
	gst_object_unref (bus);
...
gboolean
rejilla_metadata_set_uri (RejillaMetadata *self,
			  RejillaMetadataFlag flags,
			  const gchar *uri,
			  GError **error)
{
	...
	priv->flags = flags;
	if (!rejilla_metadata_set_new_uri (self, uri)) {
		if (priv->error) {
			REJILLA_UTILS_LOG ("Failed to set new URI %s", priv->error->message);
	...
	state_change = gst_element_set_state (GST_ELEMENT (priv->pipeline),
					      REJILLA_METADATA_INITIAL_STATE);
...
gboolean
rejilla_metadata_get_info_async (RejillaMetadata *self,
				 const gchar *uri,
				 RejillaMetadataFlag flags)
{
	RejillaMetadataPrivate *priv;
...
static void
rejilla_metadata_wait_cancelled (GCancellable *cancel,
				 GCond *condition)
{
	REJILLA_UTILS_LOG ("Thread waiting for retrieval end cancelled");
	g_cond_broadcast (condition);
}

void
rejilla_metadata_wait (RejillaMetadata *self,
		       GCancellable *cancel)
{
	RejillaMetadataPrivate *priv;
	GCond *condition;
	...
	REJILLA_UTILS_LOG ("Metadata lock and wait %p", self);
	...
	sig = g_signal_connect (cancel,
				"cancelled",
				G_CALLBACK (rejilla_metadata_wait_cancelled),
				condition);
...
void
rejilla_metadata_increase_listener_number (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
...
gboolean
rejilla_metadata_decrease_listener_number (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
...
const gchar *
rejilla_metadata_get_uri (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
...
RejillaMetadataFlag
rejilla_metadata_get_flags (RejillaMetadata *self)
{
	RejillaMetadataPrivate *priv;
...
gboolean
rejilla_metadata_get_result (RejillaMetadata *self,
			     RejillaMetadataInfo *info,
			     GError **error)
{
	RejillaMetadataPrivate *priv;
	...
	memset (info, 0, sizeof (RejillaMetadataInfo));
	rejilla_metadata_info_copy (info, priv->info);
	return TRUE;
}

static void
rejilla_metadata_init (RejillaMetadata *obj)
{
	RejillaMetadataPrivate *priv;

	priv = REJILLA_METADATA_PRIVATE (obj);

	priv->mutex = g_mutex_new ();
}

static void
rejilla_metadata_finalize (GObject *object)
{
	RejillaMetadataPrivate *priv;
	GSList *iter;

	priv = REJILLA_METADATA_PRIVATE (object);

	rejilla_metadata_destroy_pipeline (REJILLA_METADATA (object));
	...
	G_OBJECT_CLASS (rejilla_metadata_parent_class)->finalize (object);
}

static void
rejilla_metadata_get_property (GObject *obj,
			       guint prop_id,
			       GValue *value,
			       GParamSpec *pspec)
{
...
static void
rejilla_metadata_set_property (GObject *obj,
			       guint prop_id,
			       const GValue *value,
			       GParamSpec *pspec)
{
...
static void
rejilla_metadata_class_init (RejillaMetadataClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (RejillaMetadataPrivate));

	object_class->finalize = rejilla_metadata_finalize;
	object_class->set_property = rejilla_metadata_set_property;
	object_class->get_property = rejilla_metadata_get_property;

	rejilla_metadata_signals[COMPLETED_SIGNAL] =
	    g_signal_new ("completed",
			  G_TYPE_FROM_CLASS (klass),
			  G_SIGNAL_RUN_LAST,
			  G_STRUCT_OFFSET (RejillaMetadataClass,
					   completed),
			  NULL, NULL,
...
RejillaMetadata *
rejilla_metadata_new (void)
{
	return REJILLA_METADATA (g_object_new (REJILLA_TYPE_METADATA, NULL));
}
