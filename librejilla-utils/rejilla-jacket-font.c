...
typedef struct _RejillaJacketFontPrivate RejillaJacketFontPrivate;
struct _RejillaJacketFontPrivate
{
	GtkWidget *family;
	GtkWidget *size;
...
#define REJILLA_JACKET_FONT_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), REJILLA_TYPE_JACKET_FONT, RejillaJacketFontPrivate))
...
G_DEFINE_TYPE (RejillaJacketFont, rejilla_jacket_font, GTK_TYPE_HBOX);

static void
rejilla_jacket_font_family_changed_cb (GtkComboBox *combo,
				       RejillaJacketFont *self)
{
	g_signal_emit (self,
...
static void
rejilla_jacket_font_size_changed_cb (GtkComboBox *combo,
				     RejillaJacketFont *self)
{
	g_signal_emit (self,
...
void
rejilla_jacket_font_set_name (RejillaJacketFont *self,
			      const gchar *string)
{
	RejillaJacketFontPrivate *priv;
	...
	priv = REJILLA_JACKET_FONT_PRIVATE (self);
	...
	g_signal_handlers_block_by_func (priv->family,
					 rejilla_jacket_font_family_changed_cb,
					 self);
	...
	g_signal_handlers_block_by_func (priv->size,
					 rejilla_jacket_font_size_changed_cb,
					 self);
...
gchar *
rejilla_jacket_font_get_family (RejillaJacketFont *self)
{
	GtkTreeIter iter;
	GtkTreeModel *model;
	gchar *family = NULL;
...
guint
rejilla_jacket_font_get_size (RejillaJacketFont *self)
{
	guint size;
	GtkTreeIter iter;
	GtkTreeModel *model;
...
static void
rejilla_jacket_fill_sizes (RejillaJacketFont *self,
                           GtkListStore *store)
{
	RejillaJacketFontPrivate *priv;
	gint i;
...
static void
rejilla_jacket_fill_families (RejillaJacketFont *self,
                              GtkListStore *store)
{
	RejillaJacketFontPrivate *priv;
...
static void
rejilla_jacket_font_init (RejillaJacketFont *object)
{
	GtkListStore *store;
	GtkCellRenderer *renderer;
	RejillaJacketFontPrivate *priv;

	priv = REJILLA_JACKET_FONT_PRIVATE (object);
	...
	store = gtk_list_store_new (FAMILY_COL_NB,
				    G_TYPE_STRING,
				    G_TYPE_POINTER);
	rejilla_jacket_fill_families (object, store);
	...
	g_signal_connect (priv->family,
			  "changed",
			  G_CALLBACK (rejilla_jacket_font_family_changed_cb),
			  object);

	store = gtk_list_store_new (SIZE_COL_NB,
				    G_TYPE_STRING,
				    G_TYPE_UINT);
	rejilla_jacket_fill_sizes (object, store);
	...
	g_signal_connect (priv->size,
			  "changed",
			  G_CALLBACK (rejilla_jacket_font_size_changed_cb),
			  object);
}

static void
rejilla_jacket_font_finalize (GObject *object)
{
	G_OBJECT_CLASS (rejilla_jacket_font_parent_class)->finalize (object);
}

static void
rejilla_jacket_font_class_init (RejillaJacketFontClass *klass)
{
	GObjectClass* object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (RejillaJacketFontPrivate));

	object_class->finalize = rejilla_jacket_font_finalize;
...
{
	return g_object_new (REJILLA_TYPE_JACKET_FONT, NULL);
}
