...
typedef struct _RejillaPKPrivate RejillaPKPrivate;
struct _RejillaPKPrivate
{
...
#define REJILLA_PK_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), REJILLA_TYPE_PK, RejillaPKPrivate))

G_DEFINE_TYPE (RejillaPK, rejilla_pk, G_TYPE_OBJECT);

static void
rejilla_pk_install_missing_files_result (GObject *source_object,
					 GAsyncResult *result,
                                         gpointer user_data)
{
	GError *error = NULL;
	RejillaPKPrivate *priv = REJILLA_PK_PRIVATE (user_data);
	...
	if (priv->values == NULL) {
		REJILLA_UTILS_LOG ("%s", error->message);
		g_error_free (error);
	}
...
static void
rejilla_pk_cancelled (GCancellable *cancel,
                      RejillaPK *package)
{
	GError *error = NULL;
	RejillaPKPrivate *priv = REJILLA_PK_PRIVATE (package);
...
static gboolean
rejilla_pk_wait_for_call_end (RejillaPK *package,
                              GCancellable *cancel)
{
	RejillaPKPrivate *priv;
	GMainLoop *loop;
	gulong sig_int;

	priv = REJILLA_PK_PRIVATE (package);
	...
	sig_int = g_signal_connect (cancel,
	                            "cancelled",
	                            G_CALLBACK (rejilla_pk_cancelled),
	                            package);
...
static gboolean
rejilla_pk_connect (RejillaPK *package)
{
	RejillaPKPrivate *priv;
	GError *error = NULL;
	...
	priv->proxy = g_dbus_proxy_new_sync (priv->connection,
					     			  G_DBUS_PROXY_FLAGS_NONE,
						                 ...
						                 &error);
	if (priv->proxy == NULL) {
		REJILLA_UTILS_LOG ("Cannot connect to session service");
		return FALSE;
	}
...
static void
rejilla_pk_install_gst_plugin_result (GstInstallPluginsReturn res,
                                      gpointer user_data)
{
	RejillaPKPrivate *priv = REJILLA_PK_PRIVATE (user_data);
				     ...
				     GCancellable *cancel)
{
	...
	status = gst_install_plugins_async ((gchar **) gst_plugins->pdata,
	                                    context,
	                                    rejilla_pk_install_gst_plugin_result,
	                                    package);

	if (status == GST_INSTALL_PLUGINS_STARTED_OK)
		res = rejilla_pk_wait_for_call_end (package, cancel);
...
static gboolean
rejilla_pk_install_file_requirement (RejillaPK *package,
				     ...
				     GCancellable *cancel)
{
	...
	g_dbus_proxy_call (priv->proxy,
				      ...
				      -1,
				      NULL,
				      rejilla_pk_install_missing_files_result,
				      package);

	return rejilla_pk_wait_for_call_end (package, cancel);
}
                                ...
                                GCancellable *cancel)
{
	...
	res = rejilla_pk_install_file_requirement (package, missing_files, xid, cancel);
                                     ...
                                     GCancellable *cancel)
{
	...
	priv->res = rejilla_pk_install_file_requirement (package, missing_files, xid, cancel);
...
static void
rejilla_pk_init (RejillaPK *object)
{}

static void
rejilla_pk_finalize (GObject *object)
{
	GError *error = NULL;
	RejillaPKPrivate *priv;

	priv = REJILLA_PK_PRIVATE (object);
	...
	G_OBJECT_CLASS (rejilla_pk_parent_class)->finalize (object);
}

static void
rejilla_pk_class_init (RejillaPKClass *klass)
{
	GObjectClass* object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (RejillaPKPrivate));

	object_class->finalize = rejilla_pk_finalize;
}

RejillaPK *
rejilla_pk_new (void)
{
	return g_object_new (REJILLA_TYPE_PK, NULL);
}
