...
#define REJILLA_TYPE_IO             (rejilla_io_get_type ())
#define REJILLA_IO(obj)             (G_TYPE_CHECK_INSTANCE_CAST ((obj), REJILLA_TYPE_IO, RejillaIO))
#define REJILLA_IO_CLASS(klass)     (G_TYPE_CHECK_CLASS_CAST ((klass), REJILLA_TYPE_IO, RejillaIOClass))
#define REJILLA_IS_IO(obj)          (G_TYPE_CHECK_INSTANCE_TYPE ((obj), REJILLA_TYPE_IO))
#define REJILLA_IS_IO_CLASS(klass)  (G_TYPE_CHECK_CLASS_TYPE ((klass), REJILLA_TYPE_IO))
#define REJILLA_IO_GET_CLASS(obj)   (G_TYPE_INSTANCE_GET_CLASS ((obj), REJILLA_TYPE_IO, RejillaIOClass))

typedef struct _RejillaIOClass RejillaIOClass;
typedef struct _RejillaIO RejillaIO;

struct _RejillaIOClass
{
	RejillaAsyncTaskManagerClass parent_class;
};

struct _RejillaIO
{
	RejillaAsyncTaskManager parent_instance;
};

GType rejilla_io_get_type (void) G_GNUC_CONST;

typedef struct _RejillaIOPrivate RejillaIOPrivate;
struct _RejillaIOPrivate
{
	GMutex *lock;
	...
	RejillaIOGetParentWinCb win_callback;
	gpointer win_user_data;
};

#define REJILLA_IO_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), REJILLA_TYPE_IO, RejillaIOPrivate))
...
typedef struct _RejillaIOJobResult RejillaIOJobResult;


typedef void	(*RejillaIOJobProgressCallback)	(RejillaIOJob *job,
						 RejillaIOJobProgress *progress);

struct _RejillaIOJobProgress {
	RejillaIOJob *job;
	RejillaIOJobProgressCallback progress;

	RejillaIOPhase phase;
...
G_DEFINE_TYPE (RejillaIO, rejilla_io, REJILLA_TYPE_ASYNC_TASK_MANAGER);

static RejillaIO *singleton = NULL;

static RejillaIO *
rejilla_io_get_default ()
{
	if (singleton) {
	...
	singleton = g_object_new (REJILLA_TYPE_IO, NULL);
	g_object_ref (singleton);
	return singleton;
}
...
static gboolean
rejilla_io_job_progress_report_cb (gpointer callback_data)
{
	RejillaIOPrivate *priv;
	GSList *iter;

	priv = REJILLA_IO_PRIVATE (callback_data);
...
static void
rejilla_io_job_progress_report_start (RejillaIO *self,
				      RejillaIOJob *job,
				      RejillaIOJobProgressCallback callback)
{
	RejillaIOJobProgress *progress;
	RejillaIOPrivate *priv;

	priv = REJILLA_IO_PRIVATE (self);

	if (!job->base->methods->progress)
		return;

	progress = g_new0 (RejillaIOJobProgress, 1);
	progress->job = job;
	...
	if (!priv->progress_id)
		priv->progress_id = g_timeout_add (500, rejilla_io_job_progress_report_cb, self);
	g_mutex_unlock (priv->lock);
}

static void
rejilla_io_job_progress_report_stop (RejillaIO *self,
				     RejillaIOJob *job)
{
	RejillaIOPrivate *priv;
	GSList *iter;
...
static void
rejilla_io_unref_result_callback_data (RejillaIOResultCallbackData *data,
				       GObject *object,
...
static void
rejilla_io_job_result_free (RejillaIOJobResult *result)
{
	if (result->info)
...
static gboolean
rejilla_io_return_result_idle (gpointer callback_data)
{
	RejillaIO *self = REJILLA_IO (callback_data);
	RejillaIOResultCallbackData *data;
	RejillaIOJobResult *result;
	RejillaIOPrivate *priv;
	guint results_id;
	int i;
	...
	for (i = 0; priv->results && i < NUMBER_OF_RESULTS;) {
		 ...
		 * is to avoid RejillaDataDisc showing multiple dialogs for 
		 * various problems; like one dialog for joliet, one for deep,
		 * and one for name collision. */
		base = (RejillaIOJobBase *) result->base;
		...
		rejilla_io_unref_result_callback_data (data,
						       base->object,
						       base->methods->destroy,
						       FALSE);

		rejilla_io_job_result_free (result);
...
static void
rejilla_io_queue_result (RejillaIO *self,
			 RejillaIOJobResult *result)
{
	RejillaIOPrivate *priv;
	...
	if (!priv->results_id)
		priv->results_id = g_idle_add ((GSourceFunc) rejilla_io_return_result_idle, self);
	g_mutex_unlock (priv->lock);
}
			  ...
			  RejillaIOResultCallbackData *callback_data)
{
	...
	rejilla_io_queue_result (self, result);
	g_object_unref (self);
}
...
void
rejilla_io_set_job (RejillaIOJob *job,
		    const RejillaIOJobBase *base,
		    const gchar *uri,
		     ...
		     const RejillaAsyncTaskType *type)
{
	...
	if (job->options & REJILLA_IO_INFO_URGENT)
		rejilla_async_task_manager_queue (REJILLA_ASYNC_TASK_MANAGER (self),
						  REJILLA_ASYNC_URGENT,
						  type,
						  job);
	else if (job->options & REJILLA_IO_INFO_IDLE)
		rejilla_async_task_manager_queue (REJILLA_ASYNC_TASK_MANAGER (self),
						  REJILLA_ASYNC_IDLE,
						  type,
						  job);
		     ...
		     RejillaIOJob *job)
{
	 ...
	 * Either it's destroyed in the thread that called rejilla_io_cancel ()
	...
	if (job->callback_data) {
		...
		if (g_atomic_int_dec_and_test (&job->callback_data->ref)) {
			if (cancelled) {
				...
				rejilla_io_return_result (job->base,
							  NULL,
							  NULL,
							  NULL,
							  job->callback_data);
...
static void
rejilla_io_job_destroy (RejillaAsyncTaskManager *manager,
			gboolean cancelled,
			gpointer callback_data)
{
	RejillaIOJob *job = callback_data;
	...
	rejilla_io_job_free (cancelled, job);
}
...
struct _RejillaIOMount {
	GError *error;
	gboolean result;
	gboolean finished;
};
typedef struct _RejillaIOMount RejillaIOMount;

static void
rejilla_io_mount_enclosing_volume_cb (GObject *source,
				      GAsyncResult *result,
				      gpointer callback_data)
{
	RejillaIOMount *mount = callback_data;
...
static gboolean
rejilla_io_mount_enclosing_volume (RejillaIO *self,
				   GFile *file,
				   GCancellable *cancel,
				   GError **error)
{
	...
	g_file_mount_enclosing_volume (file,
				       ...
				       operation,
				       cancel,
				       rejilla_io_mount_enclosing_volume_cb,
				       &mount);
	...
	REJILLA_UTILS_LOG ("Parent volume is %s",
...
static gchar *
rejilla_io_check_for_parent_symlink (const gchar *escaped_uri,
				     GCancellable *cancel)
{
	GFile *parent;
	GFile *file;
...
static gchar *
rejilla_io_get_uri_from_path (GFile *file,
			      const gchar *path)
{
	gchar *uri;
...
static gboolean
rejilla_io_check_symlink_target (GFile *parent,
				 GFileInfo *info)
{
	const gchar *target;
	gchar *target_uri;
	...
	target_uri = rejilla_io_get_uri_from_path (parent, target);
	if (!target_uri)
		return FALSE;
...
typedef struct _RejillaIOMetadataTask RejillaIOMetadataTask;

struct _RejillaIOMetadataCached {
	guint64 last_modified;
	RejillaMetadataInfo *info;

	guint missing_codec_used:1;
};
typedef struct _RejillaIOMetadataCached RejillaIOMetadataCached;

static gint
rejilla_io_metadata_lookup_buffer (gconstpointer a, gconstpointer b)
{
	const RejillaIOMetadataCached *cached = a;
	const gchar *uri = b;
...
static void
rejilla_io_metadata_cached_free (RejillaIOMetadataCached *cached)
{
	rejilla_metadata_info_free (cached->info);
	g_free (cached);
}

static void
rejilla_io_set_metadata_attributes (GFileInfo *info,
				    RejillaMetadataInfo *metadata)
{
	g_file_info_set_attribute_int32 (info, REJILLA_IO_ISRC, metadata->isrc);
	g_file_info_set_attribute_uint64 (info, REJILLA_IO_LEN, metadata->len);

	if (metadata->type)
		g_file_info_set_content_type (info, metadata->type);

	if (metadata->artist)
		g_file_info_set_attribute_string (info, REJILLA_IO_ARTIST, metadata->artist);

	if (metadata->title)
		g_file_info_set_attribute_string (info, REJILLA_IO_TITLE, metadata->title);

	if (metadata->album)
		g_file_info_set_attribute_string (info, REJILLA_IO_ALBUM, metadata->album);

	if (metadata->genre)
		g_file_info_set_attribute_string (info, REJILLA_IO_GENRE, metadata->genre);

	if (metadata->composer)
		g_file_info_set_attribute_string (info, REJILLA_IO_COMPOSER, metadata->composer);

	g_file_info_set_attribute_boolean (info, REJILLA_IO_HAS_AUDIO, metadata->has_audio);
	if (metadata->has_audio) {
		if (metadata->channels)
			g_file_info_set_attribute_int32 (info, REJILLA_IO_CHANNELS, metadata->channels);

		if (metadata->rate)
			g_file_info_set_attribute_int32 (info, REJILLA_IO_RATE, metadata->rate);

		if (metadata->has_dts)
			g_file_info_set_attribute_boolean (info, REJILLA_IO_HAS_DTS, TRUE);
	}

	g_file_info_set_attribute_boolean (info, REJILLA_IO_HAS_VIDEO, metadata->has_video);
	g_file_info_set_attribute_boolean (info, REJILLA_IO_IS_SEEKABLE, metadata->is_seekable);
	if (metadata->snapshot)
		g_file_info_set_attribute_object (info, REJILLA_IO_THUMBNAIL, G_OBJECT (metadata->snapshot));

	/* FIXME: what about silences */
}

static RejillaMetadata *
rejilla_io_find_metadata (RejillaIO *self,
			  GCancellable *cancel,
			  const gchar *uri,
			  ...
			  GError **error)
{
	GSList *iter;
	RejillaIOPrivate *priv;
	RejillaMetadata *metadata;

	priv = REJILLA_IO_PRIVATE (self);

	REJILLA_UTILS_LOG ("Retrieving available metadata %s", uri);
	...
	for (iter = priv->metadata_running; iter; iter = iter->next) {
		...
		RejillaMetadataFlag metadata_flags;

		metadata = iter->data;
		metadata_uri = rejilla_metadata_get_uri (metadata);
		if (!metadata_uri) {
		...
		metadata_flags = rejilla_metadata_get_flags (metadata);
		...
		&&  !strcmp (uri, metadata_uri)) {
			 ...
			 * to waiting queue */
			REJILLA_UTILS_LOG ("Already ongoing search for %s", uri);
			rejilla_metadata_increase_listener_number (metadata);
			return metadata;
		}
	}
	...
	if (!rejilla_metadata_set_uri (metadata, flags, uri, error))
		return NULL;
...
static gboolean
rejilla_io_wait_for_metadata (RejillaIO *self,
			      GCancellable *cancel,
			      GFileInfo *info,
			      RejillaMetadata *metadata,
			      RejillaMetadataFlag flags,
			      RejillaMetadataInfo *meta_info)
{
	gboolean result;
	gboolean is_last;
	RejillaIOPrivate *priv;
	...
	rejilla_metadata_wait (metadata, cancel);
	g_mutex_lock (priv->lock_metadata);

	is_last = rejilla_metadata_decrease_listener_number (metadata);

	if (!g_cancellable_is_cancelled (cancel))
		result = rejilla_metadata_get_result (metadata, meta_info, NULL);
	else
		result = FALSE;
	...
	if (result) {
		...
		if (meta_info->has_audio || meta_info->has_video) {
			RejillaIOMetadataCached *cached;

			cached = g_new0 (RejillaIOMetadataCached, 1);
			cached->last_modified = g_file_info_get_attribute_uint64 (info, G_FILE_ATTRIBUTE_STANDARD_SIZE);

			cached->info = g_new0 (RejillaMetadataInfo, 1);
			rejilla_metadata_get_result (metadata, cached->info, NULL);

			cached->missing_codec_used = (flags & REJILLA_METADATA_FLAG_MISSING) != 0;
			...
			if (g_queue_get_length (priv->meta_buffer) > MAX_BUFFERED_META) {
				...
				rejilla_io_metadata_cached_free (cached);
			}
		}
	}

	/* Make sure it is stopped */
	REJILLA_UTILS_LOG ("Stopping metadata information retrieval (%p)", metadata);
	rejilla_metadata_cancel (metadata);
...
static gboolean
rejilla_io_get_metadata_info (RejillaIO *self,
			      GCancellable *cancel,
			      const gchar *uri,
			      GFileInfo *info,
			      RejillaMetadataFlag flags,
			      RejillaMetadataInfo *meta_info)
{
	...
	REJILLA_UTILS_LOG ("Found file with type %s", mime);
	...
	REJILLA_UTILS_LOG ("Retrieving metadata info");
	g_mutex_lock (priv->lock_metadata);
	...
	node = g_queue_find_custom (priv->meta_buffer,
				    uri,
				    rejilla_io_metadata_lookup_buffer);
	if (node) {
		guint64 last_modified;
		RejillaIOMetadataCached *cached;
		...
		if (last_modified == cached->last_modified) {
			...
			if (flags & REJILLA_METADATA_FLAG_MISSING) {
			...
			if (flags & REJILLA_METADATA_FLAG_THUMBNAIL) {
			...
			if (!refresh_cache) {
				rejilla_metadata_info_copy (meta_info, cached->info);
				g_mutex_unlock (priv->lock_metadata);
				return TRUE;
			}
		}
		...
		REJILLA_UTILS_LOG ("Updating cache information for %s", uri);
	}

	/* Find a metadata */
	metadata = rejilla_io_find_metadata (self, cancel, uri, flags, NULL);
	g_mutex_unlock (priv->lock_metadata);

	if (!metadata)
		return FALSE;

	return rejilla_io_wait_for_metadata (self,
					     cancel,
					     info,
					     metadata,
					     flags,
...
static GFileInfo *
rejilla_io_get_file_info_thread_real (RejillaAsyncTaskManager *manager,
				      GCancellable *cancel,
				      GFile *file,
				      RejillaIOFlags options,
				      GError **error)
{
	...
	if (options & REJILLA_IO_INFO_METADATA)
		strcat (attributes, "," G_FILE_ATTRIBUTE_STANDARD_SIZE);

	info = g_file_query_info (file,
				  attributes,
				  (options & REJILLA_IO_INFO_FOLLOW_SYMLINK)?G_FILE_QUERY_INFO_NONE:G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,	/* follow symlinks by default*/
				  cancel,
				  &local_error);
	if (!info) {
		if (local_error && local_error->code == G_IO_ERROR_NOT_MOUNTED) {
			...
			REJILLA_UTILS_LOG ("Starting to mount parent volume");
			...
			res = rejilla_io_mount_enclosing_volume (REJILLA_IO (manager),
								 file,
								 cancel,
								 error);
			if (!res)
			...
			return rejilla_io_get_file_info_thread_real (manager,
								     cancel,
								     file,
								     options,
								     error);
	...
	if (g_file_info_get_is_symlink (info)) {
		...
		parent = g_file_get_parent (file);
		if (!rejilla_io_check_symlink_target (parent, info)) {
			g_set_error (error,
				     REJILLA_UTILS_ERROR,
	...
	&&  (options & REJILLA_IO_INFO_METADATA)) {
		RejillaMetadataInfo metadata = { NULL };
		RejillaMetadataFlag flags;
		gboolean result;
		gchar *uri;

		flags = ((options & REJILLA_IO_INFO_METADATA_MISSING_CODEC) ? REJILLA_METADATA_FLAG_MISSING : 0)|
			((should_thumbnail) ? REJILLA_METADATA_FLAG_THUMBNAIL : 0);

		uri = g_file_get_uri (file);
		result = rejilla_io_get_metadata_info (REJILLA_IO (manager),
						       cancel,
						       uri,
						       info,
						       flags,
		...
		if (result)
			rejilla_io_set_metadata_attributes (info, &metadata);

		rejilla_metadata_info_clear (&metadata);
	}
...
rejilla_io_get_file_info_thread (RejillaAsyncTaskManager *manager,
				 GCancellable *cancel,
				 gpointer callback_data)
{
	RejillaIOJob *job = callback_data;
	...
	if (job->options & REJILLA_IO_INFO_CHECK_PARENT_SYMLINK) {
		 ...
		 * It would be much better if we could use the inode numbers provided by mate_vfs
		...
		file_uri = rejilla_io_check_for_parent_symlink (job->uri, cancel);
	}
	...
	info = rejilla_io_get_file_info_thread_real (manager,
						     cancel,
						     file,
						     job->options,
						     &error);
...
static const RejillaAsyncTaskType info_type = {
	rejilla_io_get_file_info_thread,
	rejilla_io_job_destroy
};

static void
rejilla_io_new_file_info_job (const gchar *uri,
			      const RejillaIOJobBase *base,
			      RejillaIOFlags options,
			      RejillaIOResultCallbackData *callback_data)
{
	...
	job = g_new0 (RejillaIOJob, 1);
	rejilla_io_set_job (job,
			    base,
			    uri,
			    options,
			    callback_data);

	rejilla_io_push_job (job, &info_type);
}
			  ...
			  gpointer user_data)
{
	...
	rejilla_io_new_file_info_job (uri, base, options, callback_data);
	g_object_unref (self);
}
...
struct _RejillaIOPlaylist {
	gchar *title;
	GSList *uris;
};
typedef struct _RejillaIOPlaylist RejillaIOPlaylist;

static void
rejilla_io_playlist_clear (RejillaIOPlaylist *data)
{
...
static void
rejilla_io_add_playlist_entry_parsed_cb (IdolPlParser *parser,
					 const gchar *uri,
					 GHashTable *metadata,
					 RejillaIOPlaylist *data)
{
	data->uris = g_slist_prepend (data->uris, g_strdup (uri));
}

static void
rejilla_io_start_playlist_cb (IdolPlParser *parser,
				  const gchar *uri,
				  GHashTable *metadata,
				  RejillaIOPlaylist *data)
{
...
static gboolean
rejilla_io_parse_playlist_get_uris (const gchar *uri,
				    RejillaIOPlaylist *playlist,
				    GError **error)
{
	gboolean result;
	IdolPlParser *parser;
	...
	g_signal_connect (parser,
			  "playlist-started",
			  G_CALLBACK (rejilla_io_start_playlist_cb),
			  playlist);
	g_signal_connect (parser,
			  "entry-parsed",
			  G_CALLBACK (rejilla_io_add_playlist_entry_parsed_cb),
			  playlist);

	if (g_object_class_find_property (G_OBJECT_GET_CLASS (parser), "recurse"))
		g_object_set (G_OBJECT (parser), "recurse", FALSE, NULL);

	result = idol_pl_parser_parse (parser, uri, TRUE);
...
static RejillaAsyncTaskResult
rejilla_io_parse_playlist_thread (RejillaAsyncTaskManager *manager,
				  GCancellable *cancel,
				  gpointer callback_data)
{
	GSList *iter;
	...
	GError *error = NULL;
	RejillaIOJob *job = callback_data;
	RejillaIOPlaylist data = { NULL, };

	result = rejilla_io_parse_playlist_get_uris (job->uri, &data, &error);
	if (!result) {
		rejilla_io_return_result (job->base,
					  job->uri,
					  NULL,
	...
	g_file_info_set_attribute_boolean (info,
					   REJILLA_IO_IS_PLAYLIST,
					   TRUE);
	g_file_info_set_attribute_uint32 (info,
					  REJILLA_IO_PLAYLIST_ENTRIES_NUM,
					  g_slist_length (data.uris));
	if (data.title)
		g_file_info_set_attribute_string (info,
						  REJILLA_IO_PLAYLIST_TITLE,
						  data.title);
	...
	for (iter = data.uris; iter; iter = iter->next) {
		...
		child_info = rejilla_io_get_file_info_thread_real (manager,
								   cancel,
								   file,
								   job->options,
								   NULL);
	...
	rejilla_io_playlist_clear (&data);
	return REJILLA_ASYNC_TASK_FINISHED;
}

static const RejillaAsyncTaskType playlist_type = {
	rejilla_io_parse_playlist_thread,
	rejilla_io_job_destroy
};
			   ...
			   gpointer user_data)
{
	...
	rejilla_io_push_job (job, &playlist_type);
	g_object_unref (self);
}
...
typedef struct _RejillaIOCountData RejillaIOCountData;

static void
rejilla_io_get_file_count_destroy (RejillaAsyncTaskManager *manager,
				   gboolean cancelled,
				   gpointer callback_data)
{
	RejillaIOCountData *data = callback_data;
	...
	rejilla_io_job_progress_report_stop (REJILLA_IO (manager), callback_data);

	rejilla_io_job_free (cancelled, callback_data);
}

#ifdef BUILD_PLAYLIST

static gboolean
rejilla_io_get_file_count_process_playlist (RejillaIO *self,
					    GCancellable *cancel,
					    RejillaIOCountData *data,
					    const gchar *uri)
{
	RejillaIOPlaylist playlist = {NULL, };
	GSList *iter;

	if (!rejilla_io_parse_playlist_get_uris (uri, &playlist, NULL))
		return FALSE;
	...
	for (iter = playlist.uris; iter; iter = iter->next) {
		...
		info = g_file_info_new ();
		result = rejilla_io_get_metadata_info (self,
						       cancel,
						       child_uri,
						       info,
						       ((data->job.options & REJILLA_IO_INFO_METADATA_MISSING_CODEC) ? REJILLA_METADATA_FLAG_MISSING : 0) |
						       ((data->job.options & REJILLA_IO_INFO_METADATA_THUMBNAIL) ? REJILLA_METADATA_FLAG_THUMBNAIL : 0),
						       &metadata);
	...
	rejilla_io_playlist_clear (&playlist);
	return TRUE;
}
...
static void
rejilla_io_get_file_count_process_file (RejillaIO *self,
					GCancellable *cancel,
					RejillaIOCountData *data,
					GFile *file,
					GFileInfo *info)
{
	if (data->job.options & REJILLA_IO_INFO_METADATA) {
		RejillaMetadataInfo metadata = { NULL, };
		...
		/* see if that's a playlist (and if we have recursive on). */
		else if (data->job.options & REJILLA_IO_INFO_RECURSIVE) {
			const gchar *mime;
			...
			||  !strcmp (mime, "audio/x-mpegurl"))) {
				if (!rejilla_io_get_file_count_process_playlist (self, cancel, data, child_uri))
					data->files_invalid ++;
			}
			else
				data->files_invalid ++;
...
static void
rejilla_io_get_file_count_process_directory (RejillaIO *self,
					     GCancellable *cancel,
					     RejillaIOCountData *data)
{
	GFile *file;
	...
	if ((data->job.options & REJILLA_IO_INFO_METADATA)
	&&  (data->job.options & REJILLA_IO_INFO_RECURSIVE))
		strcat (attributes, "," G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE);
	...
	enumerator = g_file_enumerate_children (file,
						attributes,
						(data->job.options & REJILLA_IO_INFO_FOLLOW_SYMLINK)?G_FILE_QUERY_INFO_NONE:G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS,	/* follow symlinks by default*/
						cancel,
						NULL);
	if (!enumerator) {
		g_object_unref (file);
	...
	while ((info = g_file_enumerator_next_file (enumerator, cancel, &error)) || error) {
		...
		if (g_file_info_get_file_type (info) == G_FILE_TYPE_REGULAR
		||  g_file_info_get_file_type (info) == G_FILE_TYPE_SYMBOLIC_LINK) {
			rejilla_io_get_file_count_process_file (self, cancel, data, child, info);
...
static gboolean
rejilla_io_get_file_count_start (RejillaIO *self,
				 GCancellable *cancel,
				 RejillaIOCountData *data,
				 const gchar *uri)
{
	...
	if (g_file_info_get_file_type (info) == G_FILE_TYPE_REGULAR
	||  g_file_info_get_file_type (info) == G_FILE_TYPE_SYMBOLIC_LINK) {
		rejilla_io_get_file_count_process_file (self, cancel, data, file, info);
		g_object_unref (file);
	}
	else if (g_file_info_get_file_type (info) == G_FILE_TYPE_DIRECTORY) {
		if (data->job.options & REJILLA_IO_INFO_RECURSIVE)
...
static void
rejilla_io_get_file_count_progress_cb (RejillaIOJob *job,
				       RejillaIOJobProgress *progress)
{
	RejillaIOCountData *data = (RejillaIOCountData *) job;
...
static RejillaAsyncTaskResult
rejilla_io_get_file_count_thread (RejillaAsyncTaskManager *manager,
				  GCancellable *cancel,
				  gpointer callback_data)
{
	RejillaIOCountData *data = callback_data;
	...
	if (data->children) {
		rejilla_io_get_file_count_process_directory (REJILLA_IO (manager), cancel, data);
		return REJILLA_ASYNC_TASK_RESCHEDULE;
	}
	...
	if (!data->uris) {
		...
		g_file_info_set_attribute_uint32 (info, REJILLA_IO_COUNT_INVALID, data->files_invalid);
		g_file_info_set_attribute_uint64 (info, REJILLA_IO_COUNT_SIZE, data->total_b);
		g_file_info_set_attribute_uint32 (info, REJILLA_IO_COUNT_NUM, data->files_num);
	...
	if (!data->progress_started) {
		rejilla_io_job_progress_report_start (REJILLA_IO (manager),
						      &data->job,
						      rejilla_io_get_file_count_progress_cb);
	...
	rejilla_io_get_file_count_start (REJILLA_IO (manager), cancel, data, uri);
	g_free (uri);

	return REJILLA_ASYNC_TASK_RESCHEDULE;
}

static const RejillaAsyncTaskType count_type = {
	rejilla_io_get_file_count_thread,
	rejilla_io_get_file_count_destroy
};
			   ...
			   gpointer user_data)
{
	...
	data = g_new0 (RejillaIOCountData, 1);
	...
	rejilla_io_push_job (REJILLA_IO_JOB (data), &count_type);
	g_object_unref (self);
}
...
struct _RejillaIOContentsData {
	RejillaIOJob job;
	GSList *children;
};
typedef struct _RejillaIOContentsData RejillaIOContentsData;

static void
rejilla_io_load_directory_destroy (RejillaAsyncTaskManager *manager,
				   gboolean cancelled,
				   gpointer callback_data)
{
	RejillaIOContentsData *data = callback_data;
...
static gboolean
rejilla_io_load_directory_playlist (RejillaIO *self,
				    GCancellable *cancel,
				    RejillaIOContentsData *data,
				    const gchar *uri,
...
static RejillaAsyncTaskResult
rejilla_io_load_directory_thread (RejillaAsyncTaskManager *manager,
				  GCancellable *cancel,
				  gpointer callback_data)
{
	gchar attributes [512] = {G_FILE_ATTRIBUTE_STANDARD_NAME "," 
	...
	if (data->job.options & REJILLA_IO_INFO_PERM)
		strcat (attributes, "," G_FILE_ATTRIBUTE_ACCESS_CAN_READ);

	if (data->job.options & REJILLA_IO_INFO_MIME)
		strcat (attributes, "," G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE);
	else if ((data->job.options & REJILLA_IO_INFO_METADATA)
	     &&  (data->job.options & REJILLA_IO_INFO_RECURSIVE))
		strcat (attributes, "," G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE);

	if (data->job.options & REJILLA_IO_INFO_ICON)
		strcat (attributes, "," G_FILE_ATTRIBUTE_STANDARD_ICON);
	...
	while ((info = g_file_enumerator_next_file (enumerator, cancel, NULL))) {
		...
		if (g_file_info_get_is_symlink (info)) {
			if (!rejilla_io_check_symlink_target (file, info)) {
		...
		if (data->job.options & REJILLA_IO_INFO_METADATA) {
			...
			if (result)
				...
				||  !strcmp (mime, "audio/x-mp3-playlist")
				||  !strcmp (mime, "audio/x-mpegurl")))
					rejilla_io_load_directory_playlist (REJILLA_IO (manager),
									    cancel,
									    data,
									    child_uri,
									    attributes);
...
static const RejillaAsyncTaskType contents_type = {
	rejilla_io_load_directory_thread,
	rejilla_io_load_directory_destroy
};
			   ...
			   gpointer user_data)
{
	...
	data = g_new0 (RejillaIOContentsData, 1);
	rejilla_io_set_job (REJILLA_IO_JOB (data),
			    base,
			    uri,
			    options,
	...
	rejilla_io_push_job (REJILLA_IO_JOB (data), &contents_type);
	g_object_unref (self);
}

static void
rejilla_io_cancel_result (RejillaIO *self,
			  RejillaIOJobResult *result)
{
	RejillaIOResultCallbackData *data;
	RejillaIOPrivate *priv;
...
static gboolean
rejilla_io_cancel_tasks_by_base_cb (RejillaAsyncTaskManager *manager,
				    gpointer callback_data,
				    gpointer user_data)
{
	RejillaIOJob *job = callback_data;
	RejillaIOJobBase *base = user_data;
...
rejilla_io_cancel_by_base (RejillaIOJobBase *base)
{
	...
	rejilla_async_task_manager_foreach_unprocessed_remove (REJILLA_ASYNC_TASK_MANAGER (self),
							       rejilla_io_cancel_tasks_by_base_cb,
							       base);

	rejilla_async_task_manager_foreach_active_remove (REJILLA_ASYNC_TASK_MANAGER (self),
							  rejilla_io_cancel_tasks_by_base_cb,
							  base);
	...
	for (iter = priv->results; iter; iter = next) {
		...
		rejilla_io_cancel_result (self, result);
	}

	g_object_unref (self);
}

struct _RejillaIOJobCompareData {
	RejillaIOCompareCallback func;
	const RejillaIOJobBase *base;
	gpointer user_data;
};
typedef struct _RejillaIOJobCompareData RejillaIOJobCompareData;

static gboolean
rejilla_io_compare_unprocessed_task (RejillaAsyncTaskManager *manager,
				     gpointer task,
				     gpointer callback_data)
{
	RejillaIOJob *job = task;
	RejillaIOJobCompareData *data = callback_data;
...
rejilla_io_find_urgent (const RejillaIOJobBase *base,
			...
			gpointer user_data)
{
	RejillaIOJobCompareData callback_data;
	RejillaIO *self = rejilla_io_get_default ();
	...
	rejilla_async_task_manager_find_urgent_task (REJILLA_ASYNC_TASK_MANAGER (self),
						     rejilla_io_compare_unprocessed_task,
						     &callback_data);
	g_object_unref (self);
                                 ...
                                 RejillaIOProgressCallback progress)
{
	...
	methods = g_new0 (RejillaIOJobCallbacks, 1);
                                  ...
                                  RejillaIOJobCallbacks *methods)
{
	...
	base = g_new0 (RejillaIOJobBase, 1);
	base->object = object;
...
rejilla_io_register (GObject *object,
		     ...
		     RejillaIOProgressCallback progress)
{
	return rejilla_io_register_with_methods (object, rejilla_io_register_job_methods (callback, destroy, progress));
}
...
static int
 rejilla_io_xid_for_metadata (gpointer user_data)
{
	RejillaIOPrivate *priv;

	priv = REJILLA_IO_PRIVATE (user_data);
	if (priv->win_callback) {
		int xid;
		GtkWindow *parent;
...
static void
rejilla_io_init (RejillaIO *object)
{
	RejillaIOPrivate *priv;
	RejillaMetadata *metadata;
	priv = REJILLA_IO_PRIVATE (object);
	 ...
	 * a thread. */
	metadata = rejilla_metadata_new ();
	priv->metadatas = g_slist_prepend (priv->metadatas, metadata);
	rejilla_metadata_set_get_xid_callback (metadata, rejilla_io_xid_for_metadata, object);
	metadata = rejilla_metadata_new ();
	priv->metadatas = g_slist_prepend (priv->metadatas, metadata);
	rejilla_metadata_set_get_xid_callback (metadata, rejilla_io_xid_for_metadata, object);
}
...
static gboolean
rejilla_io_free_async_queue (RejillaAsyncTaskManager *manager,
			     gpointer callback_data,
			     gpointer NULL_data)
{
...
static void
rejilla_io_finalize (GObject *object)
{
	RejillaIOPrivate *priv;
	GSList *iter;
	...
	rejilla_async_task_manager_foreach_unprocessed_remove (REJILLA_ASYNC_TASK_MANAGER (object),
							       rejilla_io_free_async_queue,
							       NULL);

	rejilla_async_task_manager_foreach_active_remove (REJILLA_ASYNC_TASK_MANAGER (object),
							  rejilla_io_free_async_queue,
							  NULL);
	...
	if (priv->mounted) {
		...
		for (iter = priv->mounted; iter; iter = iter->next) {
			...
			REJILLA_UTILS_LOG ("Unmountin volume");
	...
	G_OBJECT_CLASS (rejilla_io_parent_class)->finalize (object);
}

static void
rejilla_io_class_init (RejillaIOClass *klass)
{
	GObjectClass* object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (RejillaIOPrivate));

	object_class->finalize = rejilla_io_finalize;
}

static gboolean
rejilla_io_cancel (RejillaAsyncTaskManager *manager,
                   gpointer callback_data,
                   gpointer user_data)
{
	return TRUE;
...
rejilla_io_shutdown (void)
{
	...
	priv = REJILLA_IO_PRIVATE (singleton);

	rejilla_async_task_manager_foreach_unprocessed_remove (REJILLA_ASYNC_TASK_MANAGER (singleton),
							       rejilla_io_cancel,
							       NULL);

	rejilla_async_task_manager_foreach_active_remove (REJILLA_ASYNC_TASK_MANAGER (singleton),
							  rejilla_io_cancel,
							  NULL);
	...
	for (iter = priv->results; iter; iter = next) {
		...
		result = iter->data;
		next = iter->next;
		rejilla_io_cancel_result (singleton, result);
	}
