...
typedef struct _RejillaDiscMessagePrivate RejillaDiscMessagePrivate;
struct _RejillaDiscMessagePrivate
{
	GtkWidget *progress;
...
#define REJILLA_DISC_MESSAGE_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), REJILLA_TYPE_DISC_MESSAGE, RejillaDiscMessagePrivate))

G_DEFINE_TYPE (RejillaDiscMessage, rejilla_disc_message, GTK_TYPE_INFO_BAR);
...
static gboolean
rejilla_disc_message_timeout (gpointer data)
{
	RejillaDiscMessagePrivate *priv;

	priv = REJILLA_DISC_MESSAGE_PRIVATE (data);
	priv->timeout = 0;
...
void
rejilla_disc_message_set_timeout (RejillaDiscMessage *self,
				  guint mseconds)
{
	RejillaDiscMessagePrivate *priv;

	priv = REJILLA_DISC_MESSAGE_PRIVATE (self);
	...
	if (mseconds > 0)
		priv->timeout = g_timeout_add (mseconds,
					       rejilla_disc_message_timeout,
					       self);
}

void
rejilla_disc_message_set_context (RejillaDiscMessage *self,
				  guint context_id)
{
	RejillaDiscMessagePrivate *priv;
...
guint
rejilla_disc_message_get_context (RejillaDiscMessage *self)
{
	RejillaDiscMessagePrivate *priv;
...
static void
rejilla_disc_message_expander_activated_cb (GtkExpander *expander,
					    RejillaDiscMessage *self)
{
...
void
rejilla_disc_message_add_errors (RejillaDiscMessage *self,
				 GSList *errors)
{
	RejillaDiscMessagePrivate *priv;
	...
	g_signal_connect (priv->expander,
			  "activate",
			  G_CALLBACK (rejilla_disc_message_expander_activated_cb),
			  self);
...
void
rejilla_disc_message_remove_errors (RejillaDiscMessage *self)
{
	RejillaDiscMessagePrivate *priv;
...
void
rejilla_disc_message_destroy (RejillaDiscMessage *self)
{
	RejillaDiscMessagePrivate *priv;
...
void
rejilla_disc_message_remove_buttons (RejillaDiscMessage *self)
{
	RejillaDiscMessagePrivate *priv;
...
static gboolean
rejilla_disc_message_update_progress (gpointer data)
{
	RejillaDiscMessagePrivate *priv;
...
void
rejilla_disc_message_set_progress_active (RejillaDiscMessage *self,
					  gboolean active)
{
	RejillaDiscMessagePrivate *priv;
	...
	if (active) {
		...
		if (!priv->id)
			priv->id = g_timeout_add (150,
						  (GSourceFunc) rejilla_disc_message_update_progress,
						  self);
	}
	else {
...
void
rejilla_disc_message_set_primary (RejillaDiscMessage *self,
				  const gchar *message)
{
	RejillaDiscMessagePrivate *priv;
	gchar *markup;
...
void
rejilla_disc_message_set_secondary (RejillaDiscMessage *self,
				    const gchar *message)
{
	RejillaDiscMessagePrivate *priv;
...
static void
rejilla_disc_message_init (RejillaDiscMessage *object)
{
	RejillaDiscMessagePrivate *priv;
	GtkWidget *main_box;

	priv = REJILLA_DISC_MESSAGE_PRIVATE (object);
...
static void
rejilla_disc_message_finalize (GObject *object)
{
	RejillaDiscMessagePrivate *priv;
	...
	G_OBJECT_CLASS (rejilla_disc_message_parent_class)->finalize (object);
}

static void
rejilla_disc_message_class_init (RejillaDiscMessageClass *klass)
{
	GObjectClass* object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (RejillaDiscMessagePrivate));

	object_class->finalize = rejilla_disc_message_finalize;
}

GtkWidget *
rejilla_disc_message_new (void)
{
	return g_object_new (REJILLA_TYPE_DISC_MESSAGE, NULL);
}
