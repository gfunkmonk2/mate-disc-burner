...
typedef enum {
REJILLA_SCSI_DATA_MODE_1			= 0x01,
REJILLA_SCSI_DATA_MODE_2_XA			= 0x02,
REJILLA_SCSI_DATA_BLOCK_TYPE			= 0x0F
} RejillaScsiDataMode;

#if G_BYTE_ORDER == G_LITTLE_ENDIAN

struct _RejillaScsiTrackInfo {
	uchar len			[2];
...
typedef struct _RejillaScsiTrackInfo RejillaScsiTrackInfo;

#define REJILLA_SCSI_TRACK_NUM(track)		(((track).track_num_high << 8) + (track).track_num_low)
#define REJILLA_SCSI_SESSION_NUM(track)		(((track).session_num_high << 8) + (track).session_num_low)
#define REJILLA_SCSI_TRACK_NUM_PTR(track)	(((track)->track_num_high << 8) + (track)->track_num_low)
#define REJILLA_SCSI_SESSION_NUM_PTR(track)	(((track)->session_num_high << 8) + (track)->session_num_low)
