...
typedef struct _RejillaStatusDialogPrivate RejillaStatusDialogPrivate;
struct _RejillaStatusDialogPrivate
{
...
#define REJILLA_STATUS_DIALOG_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), REJILLA_TYPE_STATUS_DIALOG, RejillaStatusDialogPrivate))
...
G_DEFINE_TYPE (RejillaStatusDialog, rejilla_status_dialog, GTK_TYPE_MESSAGE_DIALOG);

enum {
	USER_INTERACTION,
	LAST_SIGNAL
};
static guint rejilla_status_dialog_signals [LAST_SIGNAL] = { 0 };
...
static void
rejilla_status_dialog_session_ready (RejillaStatusDialog *dialog)
{
	RejillaStatusDialogPrivate *priv;
...
static gboolean
rejilla_status_dialog_wait_for_ready_state (RejillaStatusDialog *dialog)
{
	RejillaStatusDialogPrivate *priv;
	...
	if (result != REJILLA_BURN_NOT_READY && result != REJILLA_BURN_RUNNING) {
		rejilla_status_dialog_session_ready (dialog);
		g_object_unref (status);
		priv->id = 0;
		return FALSE;
	}

	rejilla_status_dialog_update (dialog, status);
	g_object_unref (status);
	return TRUE;
}
...
static void
rejilla_status_dialog_wait_for_session (RejillaStatusDialog *dialog)
{
	RejillaStatus *status;
	RejillaBurnResult result;
	...
	if (rejilla_track_type_get_has_data (track_type)) {
		...
		if (REJILLA_IS_TRACK_DATA_CFG (track)) {
			g_signal_connect (track,
					  "joliet-rename",
					  G_CALLBACK (rejilla_status_dialog_joliet_rename_cb),
					  dialog);
			g_signal_connect (track,
					  "2G-file",
					  G_CALLBACK (rejilla_status_dialog_2G_file_cb),
					  dialog);
			g_signal_connect (track,
					  "deep-directory",
					  G_CALLBACK (rejilla_status_dialog_deep_directory_cb),
					  dialog);
		}
	}
	...
	priv->id = g_timeout_add (200,
				  (GSourceFunc) rejilla_status_dialog_wait_for_ready_state,
				  dialog);
}
...
static void
rejilla_status_dialog_get_property (GObject *object,
				    guint prop_id,
				    GValue *value,
				    GParamSpec *pspec)
{
...
static void
rejilla_status_dialog_finalize (GObject *object)
{
	RejillaStatusDialogPrivate *priv;
	...
	G_OBJECT_CLASS (rejilla_status_dialog_parent_class)->finalize (object);
}

static void
rejilla_status_dialog_class_init (RejillaStatusDialogClass *klass)
{
	GObjectClass* object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (RejillaStatusDialogPrivate));

	object_class->finalize = rejilla_status_dialog_finalize;
	object_class->set_property = rejilla_status_dialog_set_property;
	object_class->get_property = rejilla_status_dialog_get_property;
...
GtkWidget *
rejilla_status_dialog_new (RejillaBurnSession *session,
			   GtkWidget *parent)
{
	return g_object_new (REJILLA_TYPE_STATUS_DIALOG,
			     "session", session,
