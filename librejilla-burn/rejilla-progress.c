...
static void rejilla_burn_progress_class_init (RejillaBurnProgressClass *klass);
static void rejilla_burn_progress_init (RejillaBurnProgress *sp);
static void rejilla_burn_progress_finalize (GObject *object);

static void
rejilla_burn_progress_set_property (GObject *object,
				    guint property_id,
				    const GValue *value,
				    GParamSpec *pspec);
static void
rejilla_burn_progress_get_property (GObject *object,
				    guint property_id,
				    GValue *value,
				    GParamSpec *pspec);

struct RejillaBurnProgressPrivate {
	GtkWidget *progress;
	GtkWidget *action;
	GtkWidget *speed;
...
GType
rejilla_burn_progress_get_type ()
{
	static GType type = 0;

	if(type == 0) {
		static const GTypeInfo our_info = {
			sizeof (RejillaBurnProgressClass),
			NULL,
			NULL,
			(GClassInitFunc)rejilla_burn_progress_class_init,
			NULL,
			NULL,
			sizeof (RejillaBurnProgress),
			0,
			(GInstanceInitFunc)rejilla_burn_progress_init,
		};
...
static void
rejilla_burn_progress_class_init (RejillaBurnProgressClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);

	parent_class = g_type_class_peek_parent(klass);
	object_class->finalize = rejilla_burn_progress_finalize;
	object_class->set_property = rejilla_burn_progress_set_property;
	object_class->get_property = rejilla_burn_progress_get_property;
...
static void
rejilla_burn_progress_create_info (RejillaBurnProgress *obj)
{
	GtkWidget *label;
	GtkWidget *table;
...
void
rejilla_burn_progress_display_session_info (RejillaBurnProgress *obj,
					    glong time,
					    gint64 rate,
					    RejillaMedia media,
					    gint written)
{
	...
	if (rate > 0) {
		...
		if (media & REJILLA_MEDIUM_DVD)
			speed = (gfloat) REJILLA_RATE_TO_SPEED_DVD (rate);
		else if (media & REJILLA_MEDIUM_BD)
			speed = (gfloat) REJILLA_RATE_TO_SPEED_BD (rate);
		else
			speed = (gfloat) REJILLA_RATE_TO_SPEED_CD (rate);
...
rejilla_burn_progress_set_property (GObject *object,
				    ...
				    GParamSpec *pspec)
{
	RejillaBurnProgress *progress;

	progress = REJILLA_BURN_PROGRESS (object);
	switch (property_id) {
	case PROP_SHOW_INFO:
		if (!g_value_get_boolean (value)) {
			...
			rejilla_burn_progress_create_info (progress);
		break;
...
static void
rejilla_burn_progress_init (RejillaBurnProgress *obj)
{
	GtkWidget *box;

	obj->priv = g_new0 (RejillaBurnProgressPrivate, 1);
	...
	rejilla_burn_progress_create_info (obj);
	
	gtk_widget_show_all (GTK_WIDGET (obj));
}

static void
rejilla_burn_progress_stop_blinking (RejillaBurnProgress *self)
{
...
static void
rejilla_burn_progress_finalize (GObject *object)
{
	RejillaBurnProgress *cobj;

	cobj = REJILLA_BURN_PROGRESS (object);
...
GtkWidget *
rejilla_burn_progress_new ()
{
	RejillaBurnProgress *obj;
	
	obj = REJILLA_BURN_PROGRESS (g_object_new (REJILLA_TYPE_BURN_PROGRESS, NULL));
	
	return GTK_WIDGET (obj);
}

static gboolean
rejilla_burn_progress_pulse_cb (RejillaBurnProgress *self)
{
...
static void
rejilla_burn_progress_start_blinking (RejillaBurnProgress *self)
{
	...
	if (!self->priv->pulse_id)
		self->priv->pulse_id = g_timeout_add (150,
						      (GSourceFunc) rejilla_burn_progress_pulse_cb,
						      self);
}

void
rejilla_burn_progress_set_status (RejillaBurnProgress *self,
				  RejillaMedia media,
				  ...
				  gint64 rate)
{
	...
	if (action_progress < 0.0) {
		...
		rejilla_burn_progress_start_blinking (self);
		return;
	}

	if (self->priv->current == REJILLA_BURN_ACTION_NONE) {
		gtk_progress_bar_set_text (GTK_PROGRESS_BAR (self->priv->progress), " ");
	...
	if (self->priv->pulse_id)
		rejilla_burn_progress_stop_blinking (self);
	...
	if (self->priv->current == REJILLA_BURN_ACTION_BLANKING) {
		if (self->priv->bytes_written)
			gtk_label_set_text (GTK_LABEL (self->priv->bytes_written), " ");
		if (self->priv->speed)
			gtk_label_set_text (GTK_LABEL (self->priv->speed), " ");
...
void
rejilla_burn_progress_set_action (RejillaBurnProgress *self,
				  RejillaBurnAction action,
				  const gchar *string)
{
	gchar *final_text;

	if (action != REJILLA_BURN_ACTION_NONE) {
		if (!string)
			string = rejilla_burn_action_to_string (action);

		if (self->priv->speed_label) {
			if (action == REJILLA_BURN_ACTION_RECORDING
	...
	if (action == REJILLA_BURN_ACTION_BLANKING)
		rejilla_burn_progress_start_blinking (self);
	else if (action == REJILLA_BURN_ACTION_FINISHED)
		rejilla_burn_progress_stop_blinking (self);
}
...
{
	rejilla_burn_progress_stop_blinking (progress);

	progress->priv->current = REJILLA_BURN_ACTION_NONE;
