...
static RejillaCapsLink *
rejilla_caps_find_link_for_input (RejillaCaps *caps,
				  RejillaCaps *input)
{
	GSList *links;
...
static gint
rejilla_burn_caps_sort (gconstpointer a, gconstpointer b)
{
	const RejillaCaps *caps_a = a;
	const RejillaCaps *caps_b = b;
	gint result;
	...
	case REJILLA_TRACK_TYPE_DISC:
		if (REJILLA_MEDIUM_TYPE (caps_a->type.subtype.media) !=
		    REJILLA_MEDIUM_TYPE (caps_b->type.subtype.media))
			return ((gint32) REJILLA_MEDIUM_TYPE (caps_a->type.subtype.media) -
			        (gint32) REJILLA_MEDIUM_TYPE (caps_b->type.subtype.media));

		if ((caps_a->type.subtype.media & REJILLA_MEDIUM_DVD)
		&&  REJILLA_MEDIUM_SUBTYPE (caps_a->type.subtype.media) !=
		    REJILLA_MEDIUM_SUBTYPE (caps_b->type.subtype.media))			
			return ((gint32) REJILLA_MEDIUM_SUBTYPE (caps_a->type.subtype.media) -
			        (gint32) REJILLA_MEDIUM_SUBTYPE (caps_b->type.subtype.media));

		if (REJILLA_MEDIUM_ATTR (caps_a->type.subtype.media) !=
		    REJILLA_MEDIUM_ATTR (caps_b->type.subtype.media))
			return REJILLA_MEDIUM_ATTR (caps_a->type.subtype.media) -
			       REJILLA_MEDIUM_ATTR (caps_b->type.subtype.media);

		if (REJILLA_MEDIUM_STATUS (caps_a->type.subtype.media) !=
		    REJILLA_MEDIUM_STATUS (caps_b->type.subtype.media))
			return REJILLA_MEDIUM_STATUS (caps_a->type.subtype.media) -
			       REJILLA_MEDIUM_STATUS (caps_b->type.subtype.media);

		return (REJILLA_MEDIUM_INFO (caps_a->type.subtype.media) -
			REJILLA_MEDIUM_INFO (caps_b->type.subtype.media));
...
static RejillaCapsLink *
rejilla_caps_link_copy (RejillaCapsLink *link)
{
	RejillaCapsLink *retval;

	retval = g_new0 (RejillaCapsLink, 1);
...
static void
rejilla_caps_link_list_duplicate (RejillaCaps *dest, RejillaCaps *src)
{
	GSList *iter;
	...
	for (iter = src->links; iter; iter = iter->next) {
		...
		link = iter->data;
		dest->links = g_slist_prepend (dest->links, rejilla_caps_link_copy (link));
	}
}

static RejillaCaps *
rejilla_caps_duplicate (RejillaCaps *caps)
{
	RejillaCaps *retval;

	retval = g_new0 (RejillaCaps, 1);
	retval->flags = caps->flags;
	memcpy (&retval->type, &caps->type, sizeof (RejillaTrackType));
...
static void
rejilla_caps_replicate_modifiers (RejillaCaps *dest, RejillaCaps *src)
{
	GSList *iter;
...
static void
rejilla_caps_replicate_links (RejillaBurnCaps *self,
			      RejillaCaps *dest,
			      RejillaCaps *src)
{
	GSList *iter;

	rejilla_caps_link_list_duplicate (dest, src);

	for (iter = self->priv->caps_list; iter; iter = iter->next) {
		RejillaCaps *iter_caps;
		GSList *links;
		...
		for (links = iter_caps->links; links; links = links->next) {
			...
			link = links->data;
			if (link->caps == src) {
				RejillaCapsLink *copy;

				copy = rejilla_caps_link_copy (link);
				copy->caps = dest;
...
static void
rejilla_caps_replicate_tests (RejillaBurnCaps *self,
			      RejillaCaps *dest,
			      RejillaCaps *src)
{
	GSList *iter;
...
static void
rejilla_caps_copy_deep (RejillaBurnCaps *self,
			RejillaCaps *dest,
			RejillaCaps *src)
{
	rejilla_caps_replicate_links (self, dest, src);
	rejilla_caps_replicate_tests (self, dest, src);
	rejilla_caps_replicate_modifiers (dest,src);
}

static RejillaCaps *
rejilla_caps_duplicate_deep (RejillaBurnCaps *self,
			     RejillaCaps *caps)
{
	RejillaCaps *retval;

	retval = rejilla_caps_duplicate (caps);
	rejilla_caps_copy_deep (self, retval, caps);
	return retval;
}

static GSList *
rejilla_caps_list_check_io (RejillaBurnCaps *self,
			    GSList *list,
			    RejillaPluginIOFlag flags)
{
	...
	for (iter = list; iter; iter = iter->next) {
		RejillaCaps *caps;
		RejillaPluginIOFlag common;

		caps = iter->data;
		common = caps->flags & flags;
		if (common != caps->flags) {
			RejillaCaps *new_caps;
			...
			self->priv->caps_list = g_slist_sort (self->priv->caps_list,
							      rejilla_burn_caps_sort);

			new_caps = rejilla_caps_duplicate_deep (self, caps);
			...
			GSList *node, *next;
			RejillaPluginIOFlag complement = flags;
			...
			if (complement != REJILLA_PLUGIN_IO_NONE) {
				RejillaCaps *new_caps;
				 ...
				 * in the list */
				new_caps = rejilla_caps_duplicate (caps);
...
{
	RejillaImageFormat remaining_format;
	...
	for (iter = self->priv->caps_list; iter; iter = iter->next) {
		RejillaCaps *caps;
		RejillaImageFormat common;
		RejillaPluginIOFlag common_io;

		caps = iter->data;
		if (caps->type.type != REJILLA_TRACK_TYPE_IMAGE)
			continue;

		common_io = caps->flags & flags;
		if (common_io == REJILLA_PLUGIN_IO_NONE)
			continue;

		common = (caps->type.subtype.img_format & format);
		if (common == REJILLA_IMAGE_FORMAT_NONE)
			continue;
		...
		if (common != caps->type.subtype.img_format) {
			...
			caps = rejilla_caps_duplicate_deep (self, caps);
	...
	retval = rejilla_caps_list_check_io (self, retval, flags);

	if (remaining_format != REJILLA_IMAGE_FORMAT_NONE) {
		RejillaCaps *caps;

		caps = g_new0 (RejillaCaps, 1);
		caps->flags = flags;
		caps->type.subtype.img_format = remaining_format;
		caps->type.type = REJILLA_TRACK_TYPE_IMAGE;
		...
		REJILLA_BURN_LOG_TYPE (&caps->type, "Created new caps");
	}
			...
			RejillaStreamFormat format)
{
	...
	REJILLA_BURN_LOG_WITH_FULL_TYPE (REJILLA_TRACK_TYPE_STREAM,
					 format,
					 flags,
					 "New caps required");
	...
	for (iter = self->priv->caps_list; iter; iter = iter->next) {
		RejillaCaps *caps;
		RejillaStreamFormat common;
		RejillaPluginIOFlag common_io;
		RejillaStreamFormat common_audio;
		RejillaStreamFormat common_video;

		caps = iter->data;

		if (caps->type.type != REJILLA_TRACK_TYPE_STREAM)
			continue;
		...
		common_audio = REJILLA_STREAM_FORMAT_AUDIO (caps->type.subtype.stream_format) & 
			       REJILLA_STREAM_FORMAT_AUDIO (format);
		if (common_audio == REJILLA_AUDIO_FORMAT_NONE
		&& (REJILLA_STREAM_FORMAT_AUDIO (caps->type.subtype.stream_format)
		||  REJILLA_STREAM_FORMAT_AUDIO (format)))
			continue;

		common_video = REJILLA_STREAM_FORMAT_VIDEO (caps->type.subtype.stream_format) & 
			       REJILLA_STREAM_FORMAT_VIDEO (format);

		if (common_video == REJILLA_AUDIO_FORMAT_NONE
		&& (REJILLA_STREAM_FORMAT_VIDEO (caps->type.subtype.stream_format)
		||  REJILLA_STREAM_FORMAT_VIDEO (format)))
			continue;

		/* Likewise... that must be common */
		if ((caps->type.subtype.stream_format & REJILLA_METADATA_INFO) != (format & REJILLA_METADATA_INFO))
			continue;

		common = common_audio|common_video|(format & REJILLA_METADATA_INFO);
	...
	if (!have_the_one) {
		...
		caps->type.type = REJILLA_TRACK_TYPE_STREAM;
		...
		if (encompassing) {
			for (iter = encompassing; iter; iter = iter->next) {
				...
				iter_caps = iter->data;
				rejilla_caps_copy_deep (self, caps, iter_caps);
			}
		}
...
rejilla_caps_data_new (RejillaImageFS fs_type)
{
	...
	REJILLA_BURN_LOG_WITH_FULL_TYPE (REJILLA_TRACK_TYPE_DATA,
					 fs_type,
	...
	for (iter = self->priv->caps_list; iter; iter = iter->next) {
		RejillaCaps *caps;
		RejillaImageFS common;

		caps = iter->data;

		if (caps->type.type != REJILLA_TRACK_TYPE_DATA)
			continue;
		...
		if (common == REJILLA_IMAGE_FS_NONE)
			continue;
	...
	if (!have_the_one) {
		...
		caps->type.type = REJILLA_TRACK_TYPE_DATA;
...
static GSList *
rejilla_caps_disc_lookup_or_create (RejillaBurnCaps *self,
				    GSList *retval,
				    RejillaMedia media)
{
	GSList *iter;
	...
	caps = g_new0 (RejillaCaps, 1);
	caps->flags = REJILLA_PLUGIN_IO_ACCEPT_FILE;
	caps->type.type = REJILLA_TRACK_TYPE_DISC;
...
GSList *
rejilla_caps_disc_new (RejillaMedia type)
{
	RejillaBurnCaps *self;
	GSList *retval = NULL;
	GSList *list;
	...
	list = rejilla_media_get_all_list (type);
	for (iter = list; iter; iter = iter->next) {
		RejillaMedia medium;

		medium = GPOINTER_TO_INT (iter->data);
		retval = rejilla_caps_disc_lookup_or_create (self, retval, medium);
	}
	g_slist_free (list);
...
static void
rejilla_caps_create_links (RejillaCaps *output,
	 		   GSList *inputs,
	 		   RejillaPlugin *plugin)
{
	for (; inputs; inputs = inputs->next) {
		...
		if (output == input) {
			REJILLA_BURN_LOG ("Same input and output for link. Dropping");
			continue;
		}
		...
		&&  input->type.subtype.media == output->type.subtype.media)
			REJILLA_BURN_LOG ("Recursive link");

		link = rejilla_caps_find_link_for_input (output, input);

#if 0

		/* Mainly for extra debugging */
		REJILLA_BURN_LOG_TYPE (&output->type, "Linking");
		REJILLA_BURN_LOG_TYPE (&input->type, "to");
		REJILLA_BURN_LOG ("with %s", rejilla_plugin_get_name (plugin));

#endif

		if (!link) {
			link = g_new0 (RejillaCapsLink, 1);
			link->caps = input;
			  ...
			  GSList *inputs)
{
	...
	for (; outputs; outputs = outputs->next) {
		...
		output = outputs->data;
		rejilla_caps_create_links (output, inputs, plugin);
	}
}
			   ...
			   GSList *caps_list)
{
	for (; caps_list; caps_list = caps_list->next) {
		...
		link = rejilla_caps_find_link_for_input (caps, NULL);
		if (!link) {
			link = g_new0 (RejillaCapsLink, 1);
			link->caps = NULL;
			link->plugins = g_slist_prepend (NULL, plugin);
...
rejilla_plugin_check_caps (RejillaPlugin *plugin,
			   ...
			   GSList *caps_list)
{
	RejillaCapsTest *test = NULL;
	RejillaBurnCaps *self;
	GSList *iter;

	/* Find the the RejillaCapsTest for this type; if none create it */
	self = rejilla_burn_caps_get_default ();
			       ...
			       const gchar *name)
{
	...
	if (!name) {
		rejilla_plugin_set_group (plugin, 0);
		return;
	}
	...
	if (retval) {
		rejilla_plugin_set_group (plugin, retval);
		g_object_unref (self);
		return;
	}
	...
	rejilla_plugin_set_group (plugin, g_hash_table_size (self->priv->groups) + 1);
 ...
 * Declared in rejilla-plugin-private.h
 */
...
rejilla_plugin_can_image (RejillaPlugin *plugin)
{
	...
	self = rejilla_burn_caps_get_default ();
	for (iter = self->priv->caps_list; iter; iter = iter->next) {
		RejillaTrackDataType destination;
		RejillaCaps *caps;
		GSList *links;
