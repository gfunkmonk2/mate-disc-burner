...
static gchar *
rejilla_image_format_get_cue_file_complement (const gchar *path)
{
	FILE *file;
	gchar *ptr;
	...
	while (fgets (buffer, sizeof (buffer), file)) {
		...
		if (ptr) {
			ptr += 4;
			if (rejilla_image_format_read_path (ptr, &complement))
				break;
		}
	}
	fclose (file);
...
static gchar *
rejilla_image_format_get_toc_file_complement (const gchar *path)
{
	FILE *file;
	gchar *ptr;
	gchar *complement = NULL;
...
gchar *
rejilla_image_format_get_complement (RejillaImageFormat format,
				     const gchar *path)
{
	gchar *retval = NULL;
	...
	if (format == REJILLA_IMAGE_FORMAT_CLONE) {
		...
		/* need to parse */
		retval = rejilla_image_format_get_cue_file_complement (path);
	}
	else if (format == REJILLA_IMAGE_FORMAT_CDRDAO) {
		/* need to parse */
		retval = rejilla_image_format_get_toc_file_complement (path);
	}
	else
		retval = NULL;
...
static gchar *
rejilla_image_format_get_MSF_address (const gchar *ptr,
				      gint64 *block)
{
	gchar *next;
	gint64 address = 0;
...
gboolean
rejilla_image_format_get_iso_size (gchar *uri,
				   guint64 *blocks,
				   guint64 *size_img,
				   GCancellable *cancel,
				   GError **error)
{
	...
	if (blocks)
		*blocks = REJILLA_BYTES_TO_SECTORS (g_file_info_get_size (info), 2048);
...
gchar *
rejilla_image_format_get_default_path (RejillaImageFormat format)
{
	...
	path = g_strdup_printf ("%s/rejilla%s",
				g_get_home_dir (),
				suffix);
	...
	while (g_file_test (path, G_FILE_TEST_EXISTS)) {
		...
		path = g_strdup_printf ("%s/rejilla-%i%s",
					g_get_home_dir (),
					i,
					suffix);
		i ++;
