...
typedef struct _RejillaMkisofsBase RejillaMkisofsBase;

struct _RejillaWriteGraftData {
	RejillaMkisofsBase *base;
	GError **error;
};
typedef struct _RejillaWriteGraftData RejillaWriteGraftData;

static void
rejilla_mkisofs_base_clean (RejillaMkisofsBase *base)
{
...
static RejillaBurnResult
rejilla_mkisofs_base_write_excluded (RejillaMkisofsBase *base,
				     const gchar *uri,
				     GError **error)
{
	gint num;
	...
	if (uri && uri [0] == '/') {
		...
		REJILLA_BURN_LOG ("File not stored locally %s", uri);
		g_set_error (error,
			     REJILLA_BURN_ERROR,
	...
	if (!localpath) {
		REJILLA_BURN_LOG ("Localpath is NULL");
		return REJILLA_BURN_ERR;
	}
	...
	if (forbidden) {
		...
		REJILLA_BURN_LOG ("Escaped path %s into %s", localpath, tmp);
		g_free (localpath);
		localpath = tmp;
	}
...
static RejillaBurnResult
rejilla_mkisofs_base_write_graft (RejillaMkisofsBase *base,
				  const gchar *uri,
		       ...
		       RejillaWriteGraftData *data)
{
	...
	for (; grafts; grafts = grafts->next) {
		...
		if (!graft->path) {
			result = rejilla_mkisofs_base_write_graft (data->base,
								   graft->uri,
								   NULL,
								   data->error);
			if (result != REJILLA_BURN_OK)
...
static RejillaBurnResult
rejilla_mkisofs_base_write_grafts (RejillaMkisofsBase *base,
				   GError **error)
{
	RejillaWriteGraftData callback_data;
	gpointer result;
...
static RejillaBurnResult
rejilla_mkisofs_base_create_video_empty (RejillaMkisofsBase *base,
					 const gchar *disc_path)
{
	gchar *path;

	REJILLA_BURN_LOG ("Creating an empty Video or Audio directory");
...
static RejillaBurnResult
rejilla_mkisofs_base_empty_directory (RejillaMkisofsBase *base,
				      const gchar *disc_path,
				      GError **error)
{
	RejillaBurnResult result;
	...
	if (base->videodir) {
		...
		||  !strcmp (disc_path, "VIDEO_TS/")
		||  !strcmp (disc_path, "/VIDEO_TS/")) {
			rejilla_mkisofs_base_create_video_empty (base, disc_path);
...
static RejillaBurnResult
rejilla_mkisofs_base_process_video_graft (RejillaMkisofsBase *base,
					  RejillaGraftPt *graft,
					  GError **error)
{
	gchar *link_path;
	...
	REJILLA_BURN_LOG ("Linking %s to %s", link_path, path);
...
static RejillaBurnResult
rejilla_mkisofs_base_add_graft (RejillaMkisofsBase *base,
				RejillaGraftPt *graft,
				GError **error)
{
	GSList *list;
	...
	&& !g_str_has_prefix (graft->uri, "file://")) {
		...
		REJILLA_BURN_LOG ("File not stored locally %s", graft->uri);
		g_set_error (error,
			     REJILLA_BURN_ERROR,
			     REJILLA_BURN_ERROR_FILE_NOT_LOCAL,
			     _("The file is not stored locally"));
	...
	if (base->videodir) {
		...
		||  !strcmp (graft->path, "VIDEO_TS/")
		||  !strcmp (graft->path, "/VIDEO_TS/")) {
			res = rejilla_mkisofs_base_process_video_graft (base, graft, error);
		...
		||  !strcmp (graft->path, "AUDIO_TS/")
		||  !strcmp (graft->path, "/AUDIO_TS/"))
			return rejilla_mkisofs_base_process_video_graft (base, graft, error);
		...
		||  !strcmp (parent, "/VIDEO_TS/")) {
			if (!base->found_video_ts)
				rejilla_mkisofs_base_create_video_empty (base, parent);
...
RejillaBurnResult
rejilla_mkisofs_base_write_to_files (GSList *grafts,
				     GSList *excluded,
				     gboolean use_joliet,
				     const gchar *emptydir,
				     const gchar *videodir,
				     ...
				     GError **error)
{
	...
	if (!grafts) {
		REJILLA_BURN_LOG ("No graft passed");
		return REJILLA_BURN_ERR;
	}
	...
	for (; grafts; grafts = grafts->next) {
		...
		REJILLA_BURN_LOG ("New graft %s %s", graft->uri, graft->path);

		if (!graft->uri) {
			result = rejilla_mkisofs_base_empty_directory (&base,
								       graft->path,
								       error);
			if (result != REJILLA_BURN_OK)
				goto cleanup;
		...
		result = rejilla_mkisofs_base_add_graft (&base,
							 graft,
							 error);
		if (result != REJILLA_BURN_OK)
			goto cleanup;
	...
	result = rejilla_mkisofs_base_write_grafts (&base, error);
	if (result != REJILLA_BURN_OK)
		goto cleanup;
	...
	for (; excluded; excluded = excluded->next) {
		uri = excluded->data;
		if (!uri) {
			REJILLA_BURN_LOG ("NULL URI");
			continue;
		}

		result = rejilla_mkisofs_base_write_excluded (&base,
							      uri,
							      error);
		if (result != REJILLA_BURN_OK)
			goto cleanup;
	...
	rejilla_mkisofs_base_clean (&base);
	return REJILLA_BURN_OK;
