/*
 * rejilla-plugin-manager.c
 * This file is part of rejilla
 *
 * Copyright (C) 2007 Philippe Rouquier
 *
 * Based on pluma code (pluma/pluma-plugin-manager.c) by: 
...
typedef enum {
	REJILLA_PLUGIN_BURN_ENGINE			= 0,
	REJILLA_PLUGIN_IMAGE_ENGINE,
	REJILLA_PLUGIN_CONVERT_ENGINE,
	REJILLA_PLUGIN_MISCELLANEOUS,
	REJILLA_PLUGIN_ERROR
} RejillaPluginCategory;
...
#define REJILLA_PLUGIN_MANAGER_UI_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), REJILLA_TYPE_PLUGIN_MANAGER_UI, RejillaPluginManagerUIPrivate))

struct _RejillaPluginManagerUIPrivate
{
	GtkWidget	*tree;
...
G_DEFINE_TYPE (RejillaPluginManagerUI, rejilla_plugin_manager_ui, GTK_TYPE_VBOX)

static void plugin_manager_ui_toggle_active (GtkTreeIter *iter, GtkTreeModel *model); 
static void rejilla_plugin_manager_ui_finalize (GObject *object);

static void 
rejilla_plugin_manager_ui_class_init (RejillaPluginManagerUIClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = rejilla_plugin_manager_ui_finalize;

	g_type_class_add_private (object_class, sizeof (RejillaPluginManagerUIPrivate));
}

static RejillaPlugin *
plugin_manager_ui_get_selected_plugin (RejillaPluginManagerUI *pm)
{
	RejillaPlugin *plugin = NULL;
	GtkTreeModel *model;
	GtkTreeIter iter;
	GtkTreeSelection *selection;
	RejillaPluginManagerUIPrivate *priv;

	priv = REJILLA_PLUGIN_MANAGER_UI_GET_PRIVATE (pm);
		 ...
		 RejillaPluginManagerUI *pm)
{
	...
	authors [0] = rejilla_plugin_get_author (plugin);

	copyright = g_strdup_printf (_("Copyright %s"),
				     rejilla_plugin_get_author (plugin));

	dialog = g_object_new (GTK_TYPE_ABOUT_DIALOG,
			       "program-name", _(rejilla_plugin_get_display_name (plugin)),
			       "copyright", copyright,
			       "authors", authors,
			       "comments", rejilla_plugin_get_description (plugin),
			       "logo-icon-name", rejilla_plugin_get_icon_name (plugin),
			       NULL);
		     ...
		     RejillaPluginManagerUI *pm)
{
	...
	dialog = rejilla_plugin_option_new ();

	rejilla_plugin_option_set_plugin (REJILLA_PLUGIN_OPTION (dialog), plugin);
	gtk_window_set_transient_for (GTK_WINDOW (dialog),
				      GTK_WINDOW (gtk_widget_get_toplevel (GTK_WIDGET (pm))));
				     ...
				     gpointer           data)
{
	...
	if (error_string) {
		text = g_markup_printf_escaped ("<b>%s</b>\n%s\n<i>%s</i>",
						 ...
						 * the plugin. */
						_(rejilla_plugin_get_display_name (plugin)),
						rejilla_plugin_get_description (plugin),
						error_string);
		g_free (error_string);
	}
	else
	...
	g_object_set (G_OBJECT (cell),
		      "markup", text,
		      "sensitive", rejilla_plugin_get_gtype (plugin) != G_TYPE_NONE && !rejilla_plugin_get_compulsory (plugin),
		      NULL);
...
/*
	...
	g_object_set (G_OBJECT (cell),
		      "visible", TRUE,
		      "icon-name",
		      rejilla_plugin_get_icon_name (plugin),
		      "sensitive",
		      rejilla_plugin_get_gtype (plugin) != G_TYPE_NONE,
		      NULL);
*/
}
...
{
	RejillaPlugin *plugin;
	RejillaPluginManagerUI *pm = data;
	RejillaPluginManagerUIPrivate *priv;
	...
	gtk_widget_set_sensitive (GTK_WIDGET (priv->configure_button),
				  (plugin != NULL) && 
				   rejilla_plugin_get_next_conf_option (plugin, NULL));
}
...
static void
plugin_manager_ui_populate_lists (RejillaPluginManagerUI *pm)
{
	RejillaPluginManagerUIPrivate *priv;
	RejillaPlugin *plugin;
	...
	for (; plugins; plugins = plugins->next) {
		...
		gtk_list_store_set (model, &iter,
				    ACTIVE_COLUMN, rejilla_plugin_get_active (plugin, 0),
				    AVAILABLE_COLUMN, rejilla_plugin_get_gtype (plugin) != G_TYPE_NONE && !rejilla_plugin_get_compulsory (plugin),
				    PLUGIN_COLUMN, plugin,
				    -1);
	}
	...
	if (gtk_tree_model_get_iter_first (GTK_TREE_MODEL (model), &iter))
	{
		...
		gtk_widget_set_sensitive (GTK_WIDGET (priv->configure_button),
					  (rejilla_plugin_get_next_conf_option (plugin, NULL) != NULL));
	}
			      ...
			      gboolean      active)
{
	...
	||  rejilla_plugin_get_compulsory (plugin))
		return FALSE;
...
static void
plugin_manager_ui_set_active_all (RejillaPluginManagerUI *pm,
				  gboolean            active)
{
	RejillaPluginManagerUIPrivate *priv;
	GtkTreeModel *model;
		...
		gpointer      data)
{
	...
	normalized_string = g_utf8_normalize (_(rejilla_plugin_get_display_name (plugin)),
					      -1,
					      G_NORMALIZE_ALL);
...
static GtkWidget *
create_tree_popup_menu (RejillaPluginManagerUI *pm)
{
	GtkWidget *menu;
	GtkWidget *item;
	GtkWidget *image;
	...
	gtk_widget_set_sensitive (item,
				  rejilla_plugin_get_gtype (plugin) != G_TYPE_NONE && !rejilla_plugin_get_compulsory (plugin));	
	gtk_check_menu_item_set_active (GTK_CHECK_MENU_ITEM (item),
					rejilla_plugin_get_active (plugin, 0));
...
static void
tree_popup_menu_detach (RejillaPluginManagerUI *pm,
			GtkMenu            *menu)
{
	RejillaPluginManagerUIPrivate *priv;
		      ...
		      gpointer      user_data)
{
	...
	active1 = rejilla_plugin_get_gtype (plugin1) != G_TYPE_NONE && !rejilla_plugin_get_compulsory (plugin1);
	active2 = rejilla_plugin_get_gtype (plugin2) != G_TYPE_NONE && !rejilla_plugin_get_compulsory (plugin2);
	...
	return g_utf8_collate (_(rejilla_plugin_get_display_name (plugin1)),
			       _(rejilla_plugin_get_display_name (plugin2)));
}

static void
plugin_manager_ui_construct_tree (RejillaPluginManagerUI *pm)
{
	RejillaPluginManagerUIPrivate *priv;
	GtkTreeViewColumn *column;
	GtkCellRenderer *cell;
...
static void 
rejilla_plugin_manager_ui_init (RejillaPluginManagerUI *pm)
{
	GtkWidget *vbox;
	GtkWidget *hbox;
	GtkWidget *viewport;
	...
	priv->about_button = rejilla_utils_make_button (_("_About"),
							NULL,
							GTK_STOCK_ABOUT,
							GTK_ICON_SIZE_BUTTON);
	gtk_container_add (GTK_CONTAINER (vbuttonbox), priv->about_button);

	priv->configure_button = rejilla_utils_make_button (_("C_onfigure"),
							    NULL,
	...
	priv->plugins = rejilla_burn_library_get_plugins_list ();
...
static void
rejilla_plugin_manager_ui_finalize (GObject *object)
{
	RejillaPluginManagerUI *pm = REJILLA_PLUGIN_MANAGER_UI (object);
	RejillaPluginManagerUIPrivate *priv;
	...
	G_OBJECT_CLASS (rejilla_plugin_manager_ui_parent_class)->finalize (object);

}

GtkWidget *rejilla_plugin_manager_ui_new (void)
{
	return g_object_new (REJILLA_TYPE_PLUGIN_MANAGER_UI,0);
}
