...
#include "rejilla-split-dialog.h"
#include "rejilla-video-tree-model.h"

#include "eggtreemultidnd.h"

static RejillaDiscResult
rejilla_audio_disc_set_session_contents (RejillaDisc *disc,
					 RejillaBurnSession *session);

static RejillaDiscResult
rejilla_audio_disc_add_uri (RejillaDisc *disc,
			    const char *uri);
static void
rejilla_audio_disc_delete_selected (RejillaDisc *disc);


static guint
rejilla_audio_disc_add_ui (RejillaDisc *disc,
			   GtkUIManager *manager,
			   GtkWidget *message);

static gboolean
rejilla_audio_disc_button_pressed_cb (GtkTreeView *tree,
				      GdkEventButton *event,
				      RejillaAudioDisc *disc);
static gboolean
rejilla_audio_disc_key_released_cb (GtkTreeView *tree,
				    GdkEventKey *event,
				    RejillaAudioDisc *disc);
static void
rejilla_audio_disc_display_edited_cb (GtkCellRendererText *cellrenderertext,
				      gchar *path_string,
				      gchar *text,
				      RejillaAudioDisc *disc);
static void
rejilla_audio_disc_display_editing_started_cb (GtkCellRenderer *renderer,
					       GtkCellEditable *editable,
					       gchar *path,
					       RejillaAudioDisc *disc);
static void
rejilla_audio_disc_display_editing_canceled_cb (GtkCellRenderer *renderer,
						 RejillaAudioDisc *disc);

static void
rejilla_audio_disc_edit_information_cb (GtkAction *action,
					RejillaAudioDisc *disc);
static void
rejilla_audio_disc_open_activated_cb (GtkAction *action,
				      RejillaAudioDisc *disc);
static void
rejilla_audio_disc_delete_activated_cb (GtkAction *action,
					RejillaDisc *disc);
static void
rejilla_audio_disc_paste_activated_cb (GtkAction *action,
				       RejillaAudioDisc *disc);
static gboolean
rejilla_audio_disc_get_selected_uri (RejillaDisc *disc,
				     gchar **uri);
static gboolean
rejilla_audio_disc_get_boundaries (RejillaDisc *disc,
				   gint64 *start,
				   gint64 *end);

static void
rejilla_audio_disc_add_pause_cb (GtkAction *action, RejillaAudioDisc *disc);
static void
rejilla_audio_disc_split_cb (GtkAction *action, RejillaAudioDisc *disc);

static void
rejilla_audio_disc_selection_changed (GtkTreeSelection *selection, RejillaAudioDisc *disc);

struct _RejillaAudioDiscPrivate {
	RejillaIOJobBase *add_dir;
	RejillaIOJobBase *add_playlist;
...
#define REJILLA_AUDIO_DISC_CONTEXT		1000

static void rejilla_audio_disc_iface_disc_init (RejillaDiscIface *iface);

G_DEFINE_TYPE_WITH_CODE (RejillaAudioDisc,
			 rejilla_audio_disc,
			 GTK_TYPE_VBOX,
			 G_IMPLEMENT_INTERFACE (REJILLA_TYPE_DISC,
					        rejilla_audio_disc_iface_disc_init));
static gboolean
rejilla_audio_disc_is_empty (RejillaDisc *disc)
{
	GtkTreeModel *model;

	model = gtk_tree_view_get_model (GTK_TREE_VIEW (REJILLA_AUDIO_DISC (disc)->priv->tree));
	if (!model)
		return FALSE;
...
static void
rejilla_audio_disc_iface_disc_init (RejillaDiscIface *iface)
{
	iface->add_uri = rejilla_audio_disc_add_uri;
	iface->delete_selected = rejilla_audio_disc_delete_selected;

	iface->is_empty = rejilla_audio_disc_is_empty;

	iface->set_session_contents = rejilla_audio_disc_set_session_contents;
	iface->get_selected_uri = rejilla_audio_disc_get_selected_uri;
	iface->get_boundaries = rejilla_audio_disc_get_boundaries;
	iface->add_ui = rejilla_audio_disc_add_ui;
}

static void
rejilla_audio_disc_get_property (GObject * object,
				 guint prop_id,
				 GValue * value,
				 GParamSpec * pspec)
{
	...
	disc = REJILLA_AUDIO_DISC (object);
...
static void
rejilla_audio_disc_set_property (GObject * object,
				 guint prop_id,
				 const GValue * value,
				 GParamSpec * pspec)
{
...
rejilla_audio_disc_add_ui (RejillaDisc *disc,
			   ...
			   GtkWidget *message)
{
	RejillaAudioDisc *audio_disc;
	GError *error = NULL;
	GtkAction *action;
	guint merge_id;

	audio_disc = REJILLA_AUDIO_DISC (disc);
	...
	if (!audio_disc->priv->disc_group) {
		audio_disc->priv->disc_group = gtk_action_group_new (REJILLA_DISC_ACTION "-audio");
...
static gboolean
rejilla_audio_disc_selection_function (GtkTreeSelection *selection,
				       GtkTreeModel *model,
...
/*	if (track)
		gtk_list_store_set (GTK_LIST_STORE (model), &iter,
				    REJILLA_VIDEO_TREE_MODEL_EDITABLE, (is_selected == FALSE),
				    -1);
*/
	return TRUE;
}
...
static void
rejilla_audio_disc_init (RejillaAudioDisc *obj)
{
	...
	obj->priv = g_new0 (RejillaAudioDiscPrivate, 1);
	...
	g_signal_connect (gtk_tree_view_get_selection (GTK_TREE_VIEW (obj->priv->tree)),
			  "changed",
			  G_CALLBACK (rejilla_audio_disc_selection_changed),
			  obj);
	...
	g_signal_connect (G_OBJECT (obj->priv->tree),
			  "button-press-event",
			  G_CALLBACK (rejilla_audio_disc_button_pressed_cb),
			  obj);
	g_signal_connect (G_OBJECT (obj->priv->tree),
			  "key-release-event",
			  G_CALLBACK (rejilla_audio_disc_key_released_cb),
			  obj);

	selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (obj->priv->tree));
	gtk_tree_selection_set_mode (selection,GTK_SELECTION_MULTIPLE);
	gtk_tree_selection_set_select_function (selection, rejilla_audio_disc_selection_function, NULL, NULL);
	...
	column = gtk_tree_view_column_new_with_attributes (_("Track"), renderer,
							   "text", REJILLA_VIDEO_TREE_MODEL_INDEX,
							   NULL);
	...
	gtk_tree_view_column_add_attribute (column, renderer,
					    "icon-name", REJILLA_VIDEO_TREE_MODEL_ICON_NAME);

	renderer = gtk_cell_renderer_text_new ();
	g_object_set_data (G_OBJECT (renderer), COL_KEY, REJILLA_TRACK_STREAM_TITLE_TAG);
	...
	g_signal_connect (G_OBJECT (renderer), "edited",
			  G_CALLBACK (rejilla_audio_disc_display_edited_cb), obj);
	g_signal_connect (G_OBJECT (renderer), "editing-started",
			  G_CALLBACK (rejilla_audio_disc_display_editing_started_cb), obj);
	g_signal_connect (G_OBJECT (renderer), "editing-canceled",
			  G_CALLBACK (rejilla_audio_disc_display_editing_canceled_cb), obj);

	gtk_tree_view_column_pack_end (column, renderer, TRUE);
	gtk_tree_view_column_add_attribute (column, renderer,
					    "text", REJILLA_VIDEO_TREE_MODEL_NAME);
	gtk_tree_view_column_add_attribute (column, renderer,
					    "style", REJILLA_VIDEO_TREE_MODEL_STYLE);
	gtk_tree_view_column_add_attribute (column, renderer,
					    "weight", REJILLA_VIDEO_TREE_MODEL_WEIGHT);
	gtk_tree_view_column_add_attribute (column, renderer,
	...
	renderer = gtk_cell_renderer_text_new ();
	g_object_set_data (G_OBJECT (renderer), COL_KEY, REJILLA_TRACK_STREAM_ARTIST_TAG);
	g_object_set (G_OBJECT (renderer),
	...
	column = gtk_tree_view_column_new_with_attributes (_("Artist"), renderer,
							   "text", REJILLA_VIDEO_TREE_MODEL_ARTIST,
							   NULL);
	gtk_tree_view_append_column (GTK_TREE_VIEW (obj->priv->tree), column);
	gtk_tree_view_column_set_resizable (column, TRUE);
...
static void
rejilla_audio_disc_reset_real (RejillaAudioDisc *disc)
{
	rejilla_io_cancel_by_base (disc->priv->add_dir);
	rejilla_io_cancel_by_base (disc->priv->add_playlist);
	...
	if (disc->priv->message)
		rejilla_notify_message_remove (disc->priv->message, REJILLA_AUDIO_DISC_CONTEXT);
}

static void
rejilla_audio_disc_finalize (GObject *object)
{
	RejillaAudioDisc *cobj;
	cobj = REJILLA_AUDIO_DISC(object);
	
	rejilla_audio_disc_reset_real (cobj);

	rejilla_io_job_base_free (cobj->priv->add_dir);
	rejilla_io_job_base_free (cobj->priv->add_playlist);
...
static void
rejilla_audio_disc_add_gap (RejillaAudioDisc *disc,
			    GtkTreePath *treepath,
			    gint64 gap)
{
	GtkTreeModel *model;
...
static void
rejilla_audio_disc_short_track_dialog (RejillaAudioDisc *disc)
{
...
static RejillaDiscResult
rejilla_audio_disc_add_uri_real (RejillaAudioDisc *disc,
				 const gchar *uri,
				 gint pos,
				 gint64 gap_sectors,
				 gint64 start,
				 ...
				 GtkTreePath **path_return)
{
	...
	session = rejilla_video_tree_model_get_session (REJILLA_VIDEO_TREE_MODEL (store));
	...
	rejilla_track_stream_set_boundaries (REJILLA_TRACK_STREAM (track),
					     ...
					     REJILLA_SECTORS_TO_DURATION (gap_sectors));
	...
	if (path_return)
		*path_return = rejilla_video_tree_model_track_to_path (REJILLA_VIDEO_TREE_MODEL (store), REJILLA_TRACK (track));
...
static void
rejilla_audio_disc_file_type_error_dialog (RejillaAudioDisc *disc,
					   const gchar *uri)
{
	gchar *primary;
	gchar *name;
...
static gboolean
rejilla_audio_disc_video_file_dialog (RejillaAudioDisc *disc,
				      const gchar *uri)
{
	GtkWidget *dialog;
...
static void
rejilla_audio_disc_result (GObject *obj,
			   GError *error,
			   const gchar *uri,
			   GFileInfo *info,
			   gpointer null_data)
{
	RejillaAudioDisc *disc = REJILLA_AUDIO_DISC (obj);

	if (error)
		return;

	/* we silently ignore the title and any error */
	if (g_file_info_get_attribute_string (info, REJILLA_IO_PLAYLIST_TITLE))
		return;

	if (!g_file_info_get_attribute_boolean (info, REJILLA_IO_HAS_AUDIO))
		return;

	if (g_file_info_get_attribute_boolean (info, REJILLA_IO_HAS_VIDEO)
	&& !rejilla_audio_disc_video_file_dialog (disc, uri))
		return;

	rejilla_audio_disc_add_uri_real (disc,
					 uri,
					 -1,
					 -1,
					 -1,
...
static RejillaDiscResult
rejilla_audio_disc_add_directory_contents (RejillaAudioDisc *disc,
					   const gchar *uri)
{
	if (!disc->priv->add_dir)
		disc->priv->add_dir = rejilla_io_register (G_OBJECT (disc),
							   rejilla_audio_disc_result,
							   NULL,
							   NULL);
...
static gboolean
rejilla_audio_disc_add_dir (RejillaAudioDisc *disc, const gchar *uri)
{
	gint answer;
	GtkWidget *dialog;
...
static RejillaDiscResult
rejilla_audio_disc_add_playlist (RejillaAudioDisc *disc,
				 const gchar *uri)
{
	if (!disc->priv->add_playlist)
		disc->priv->add_playlist = rejilla_io_register (G_OBJECT (disc),
								rejilla_audio_disc_result,
								NULL,
								NULL);

	rejilla_io_parse_playlist (uri,
...
/**************************** New Row ******************************************/
static void
rejilla_audio_disc_unreadable_dialog (RejillaAudioDisc *disc,
				      const gchar *uri,
				      GError *error)
{
	gchar *primary;
...
static void
rejilla_audio_disc_wav_dts_response_cb (GtkButton *button,
                                        ...
                                        RejillaAudioDisc *disc)
{
	...
	if (response == GTK_RESPONSE_OK)
		rejilla_burn_session_tag_add_int (REJILLA_BURN_SESSION (session),
		                                  REJILLA_SESSION_STREAM_AUDIO_FORMAT,
		                                  REJILLA_AUDIO_FORMAT_DTS);
}

static void
rejilla_audio_disc_wav_dts_file_dialog (RejillaAudioDisc *disc)
{
	GtkWidget *message;
	RejillaSessionCfg *session;
	GtkTreeModel *model;

	if (rejilla_notify_get_message_by_context_id (disc->priv->message,
	                                              REJILLA_AUDIO_DISC_CONTEXT))
		return;
	...
	message = rejilla_notify_message_add (disc->priv->message,
	...
	g_signal_connect (REJILLA_DISC_MESSAGE (message),
			  "response",
			  G_CALLBACK (rejilla_audio_disc_wav_dts_response_cb),
			  disc);
}

static void
rejilla_audio_disc_session_changed (RejillaSessionCfg *session,
				    RejillaAudioDisc *self)
{
	GSList *next;
	GSList *tracks;
	gboolean notready;
	...
	for (; tracks; tracks = next) {
		...
		if (result == REJILLA_BURN_ERR) {
			...
			if (!error)
				rejilla_audio_disc_file_type_error_dialog (self, uri);
			else if (error->code == REJILLA_BURN_ERROR_FILE_FOLDER) {
				res = rejilla_audio_disc_add_dir (self, uri);
				if (res)
					rejilla_audio_disc_add_directory_contents (self, uri);
			}

#if BUILD_PLAYLIST
			else if (error->code == REJILLA_BURN_ERROR_FILE_PLAYLIST) {
				/* This is a supported playlist */
				rejilla_audio_disc_add_playlist (self, uri);
			}
#endif
				...
				rejilla_audio_disc_unreadable_dialog (self,
								      uri,
								      error);
		...
		if (!REJILLA_STREAM_FORMAT_AUDIO (format)) {
			gchar *uri;
		...
		if ((format & REJILLA_AUDIO_FORMAT_DTS) != 0)
			should_use_dts = TRUE;
		...
		if (REJILLA_STREAM_FORMAT_HAS_VIDEO (format)) {
			...
			uri = rejilla_track_stream_get_source (track, TRUE);
			res = rejilla_audio_disc_video_file_dialog (self, uri);
			if (res)
				rejilla_burn_session_remove_track (REJILLA_BURN_SESSION (session),
								   REJILLA_TRACK (track));
			g_free (uri);
	...
	if (should_use_dts
	&&  rejilla_burn_session_tag_lookup (REJILLA_BURN_SESSION (session), REJILLA_SESSION_STREAM_AUDIO_FORMAT, NULL) != REJILLA_BURN_OK)
		rejilla_audio_disc_wav_dts_file_dialog (self);
}
			    ...
			    const gchar *uri)
{
	...
	audio_disc = REJILLA_AUDIO_DISC (disc);
	result = rejilla_audio_disc_add_uri_real (audio_disc,
						  uri,
						  -1,
						  0,
						  -1,
...
static void
rejilla_audio_disc_remove (RejillaAudioDisc *disc,
			   GtkTreePath *treepath)
{
	RejillaSessionCfg *session;
	GtkTreeModel *model;
	...
	gtk_tree_model_get (model, &iter,
			    REJILLA_VIDEO_TREE_MODEL_IS_GAP, &is_gap,
			    -1);
...
{
	GtkTreeSelection *selection;
	RejillaAudioDisc *audio;
	GtkTreePath *treepath;
	GtkTreeModel *model;
	GList *list;

	audio = REJILLA_AUDIO_DISC (disc);
	...
	for (; list; list = g_list_remove (list, treepath)) {
		treepath = list->data;
		rejilla_audio_disc_remove (audio, treepath);
		gtk_tree_path_free (treepath);
	}
					 ...
					 RejillaBurnSession *session)
{
	...
	if (audio->priv->add_dir)
		rejilla_io_cancel_by_base (audio->priv->add_dir);

	if (audio->priv->add_playlist)
		rejilla_io_cancel_by_base (audio->priv->add_playlist);
	...
	if (current_model) {
		...
		if (current_session)
			g_signal_handlers_disconnect_by_func (current_session,
							      rejilla_audio_disc_session_changed,
							      disc);
	}
	...
	g_signal_connect (session,
			  "is-valid",
			  G_CALLBACK (rejilla_audio_disc_session_changed),
			  disc);
...
/********************************** Cell Editing *******************************/
static void
rejilla_audio_disc_display_edited_cb (GtkCellRendererText *renderer,
				      gchar *path_string,
				      gchar *text,
				      RejillaAudioDisc *disc)
{
...
/********************************** Pause **************************************/
static void
rejilla_audio_disc_add_pause (RejillaAudioDisc *disc)
{
	GtkTreeSelection *selection;
	GtkTreePath *treepath;
	GtkTreeModel *model;
	...
	for (iter = references; iter; iter = iter->next) {
		...
		rejilla_audio_disc_add_gap (disc, treepath, 2 * GST_SECOND);
		gtk_tree_path_free (treepath);
	}
...
static void
rejilla_audio_disc_add_pause_cb (GtkAction *action,
				 RejillaAudioDisc *disc)
{
	rejilla_audio_disc_add_pause (disc);
}

static void
rejilla_audio_disc_add_slices (RejillaAudioDisc *disc,
			       GtkTreePath *treepath,
			       GSList *slices)
{
	RejillaSessionCfg *session;
	RejillaStreamFormat format;
	RejillaAudioSlice *slice;
	RejillaTrack *track;
	GtkTreeModel *model;
	GSList *iter;
	gchar *uri;
	...
	for (iter = slices->next; iter; iter = iter->next) {
		RejillaTrackStream *new_track;

		slice = iter->data;

		new_track = rejilla_track_stream_new ();
		rejilla_track_stream_set_source (new_track, uri);
		rejilla_track_stream_set_format (new_track, format);
		rejilla_track_stream_set_boundaries (new_track,
						     slice->start,
						     slice->end,
						     rejilla_track_stream_get_gap (REJILLA_TRACK_STREAM (track)));
		rejilla_track_tag_copy_missing (REJILLA_TRACK (new_track),
						REJILLA_TRACK (track));
...
static void
rejilla_audio_disc_split (RejillaAudioDisc *disc)
{
	GtkTreeSelection *selection;
	RejillaSessionCfg *session;
	GtkTreePath *treepath;
	...
	dialog = rejilla_split_dialog_new ();
	...
	uri = rejilla_track_stream_get_source (REJILLA_TRACK_STREAM (track), TRUE);
	rejilla_split_dialog_set_uri (REJILLA_SPLIT_DIALOG (dialog),
	                              uri,
	...
	rejilla_split_dialog_set_boundaries (REJILLA_SPLIT_DIALOG (dialog),
	...
	slices = rejilla_split_dialog_get_slices (REJILLA_SPLIT_DIALOG (dialog));
	gtk_widget_destroy (dialog);

	rejilla_audio_disc_add_slices (disc, treepath, slices);
...
static void
rejilla_audio_disc_split_cb (GtkAction *action,
			     RejillaAudioDisc *disc)
{
	rejilla_audio_disc_split (disc);
}

static void
rejilla_audio_disc_selection_changed (GtkTreeSelection *selection,
				      RejillaAudioDisc *disc)
{
	...
	rejilla_disc_selection_changed (REJILLA_DISC (disc));
...
/********************************** Menus **************************************/
static void
rejilla_audio_disc_open_file (RejillaAudioDisc *disc)
{
	char *uri;
	GList *item, *list;
	GSList *uris = NULL;
...
static gboolean
rejilla_audio_disc_rename_songs (GtkTreeModel *model,
				 GtkTreeIter *iter,
				 GtkTreePath *treepath,
...
static void
rejilla_audio_disc_edit_multi_song_properties (RejillaAudioDisc *disc,
					       GList *list)
{
	gint isrc;
	gint64 gap;
	...
	rejilla_multi_song_props_set_rename_callback (REJILLA_MULTI_SONG_PROPS (props),
						      ...
						      rejilla_audio_disc_rename_songs);
...
static void
rejilla_audio_disc_edit_single_song_properties (RejillaAudioDisc *disc,
						GtkTreePath *treepath)
{
	gint64 gap;
	gint isrc;
	...
	if (!track)
        ...
        gtk_tree_model_get_iter (model, &iter, treepath);
        gtk_tree_model_get (model, &iter,
                            REJILLA_VIDEO_TREE_MODEL_INDEX_NUM, &track_num,
                            -1);
	...
	rejilla_song_props_set_properties (REJILLA_SONG_PROPS (props),
					   track_num,
					   rejilla_track_tag_lookup_string (REJILLA_TRACK (track), REJILLA_TRACK_STREAM_ARTIST_TAG),
					   rejilla_track_tag_lookup_string (REJILLA_TRACK (track), REJILLA_TRACK_STREAM_TITLE_TAG),
					   rejilla_track_tag_lookup_string (REJILLA_TRACK (track), REJILLA_TRACK_STREAM_COMPOSER_TAG),
					   rejilla_track_tag_lookup_int (REJILLA_TRACK (track), REJILLA_TRACK_STREAM_ISRC_TAG),
					   length,
					   rejilla_track_stream_get_start (REJILLA_TRACK_STREAM (track)),
					   rejilla_track_stream_get_end (REJILLA_TRACK_STREAM (track)),
					   rejilla_track_stream_get_gap (REJILLA_TRACK_STREAM (track)));
	...
	if (end - start + REJILLA_SECTORS_TO_DURATION (gap) < REJILLA_MIN_STREAM_LENGTH)
		rejilla_audio_disc_short_track_dialog (disc);
...
static void
rejilla_audio_disc_edit_song_properties (RejillaAudioDisc *disc,
					 GList *list)
{
	GList *item;
	gint song_num;
	...
	if (song_num == 1)
		rejilla_audio_disc_edit_single_song_properties (disc, real_list->data);
	else
		rejilla_audio_disc_edit_multi_song_properties (disc, real_list);
...
rejilla_audio_disc_open_activated_cb (GtkAction *action,
				      RejillaAudioDisc *disc)
{
	rejilla_audio_disc_open_file (disc);
}
					...
					RejillaAudioDisc *disc)
{
	...
	rejilla_audio_disc_edit_song_properties (disc, list);
...
rejilla_audio_disc_delete_activated_cb (GtkAction *action,
					RejillaDisc *disc)
{
	rejilla_audio_disc_delete_selected (disc);
}

static void
rejilla_audio_disc_clipboard_text_cb (GtkClipboard *clipboard,
				      const gchar *text,
				      RejillaAudioDisc *disc)
{
	gchar **array;
...
static void
rejilla_audio_disc_clipboard_targets_cb (GtkClipboard *clipboard,
					 GdkAtom *atoms,
					 gint n_atoms,
					 RejillaAudioDisc *disc)
{
	if (rejilla_clipboard_selection_may_have_uri (atoms, n_atoms))
		gtk_clipboard_request_text (clipboard,
					    (GtkClipboardTextReceivedFunc) rejilla_audio_disc_clipboard_text_cb,
					    disc);
}
				       ...
				       RejillaAudioDisc *disc)
{
	...
	gtk_clipboard_request_targets (clipboard,
				       (GtkClipboardTargetsReceivedFunc) rejilla_audio_disc_clipboard_targets_cb,
				       disc);
}
...
/********************************** key press event ****************************/
static void
rejilla_audio_disc_rename_activated (RejillaAudioDisc *disc)
{
	GtkTreeSelection *selection;
	GtkTreeViewColumn *column;
	GtkTreePath *treepath;
				    ...
				    RejillaAudioDisc *disc)
{
	...
	if (event->keyval == GDK_KEY_KP_Delete || event->keyval == GDK_KEY_Delete) {
		rejilla_audio_disc_delete_selected (REJILLA_DISC (disc));
	}
	else if (event->keyval == GDK_KEY_F2)
		rejilla_audio_disc_rename_activated (disc);
				     ...
				     gchar **uri)
{
	...
	model = gtk_tree_view_get_model (GTK_TREE_VIEW (audio->priv->tree));
        track = rejilla_video_tree_model_path_to_track (REJILLA_VIDEO_TREE_MODEL (model),
...
static void
rejilla_audio_disc_class_init (RejillaAudioDiscClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);

	parent_class = g_type_class_peek_parent (klass);
	object_class->finalize = rejilla_audio_disc_finalize;
	object_class->set_property = rejilla_audio_disc_set_property;
	object_class->get_property = rejilla_audio_disc_get_property;
...
GtkWidget *
rejilla_audio_disc_new ()
{
	RejillaAudioDisc *obj;
	
	obj = REJILLA_AUDIO_DISC (g_object_new (REJILLA_TYPE_AUDIO_DISC, NULL));
