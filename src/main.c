...
static RejillaApp *current_app = NULL;

/**
 * This is actually declared in rejilla-app.h
 */
...
main (int argc, char **argv)
{
	 ...
	 * in a usable state to display our error messages while rejilla
	...
	g_option_context_add_group (context, rejilla_media_get_option_group ());
	g_option_context_add_group (context, rejilla_burn_library_get_option_group ());
	...
	if (cmd_line_options.not_unique == FALSE) {
		...
		uapp = unique_app_new ("org.mate.Rejilla", NULL);
	...
	rejilla_burn_library_start (&argc, &argv);
	rejilla_enable_multi_DND ();

	current_app = rejilla_app_new (uapp);
	g_object_unref (uapp);
	if (current_app == NULL)
		return 1;

	rejilla_cli_apply_options (current_app);
