...
typedef struct _RejillaSplitDialogPrivate RejillaSplitDialogPrivate;
struct _RejillaSplitDialogPrivate
{
	GtkWidget *cut;
...
#define REJILLA_SPLIT_DIALOG_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), REJILLA_TYPE_SPLIT_DIALOG, RejillaSplitDialogPrivate))

G_DEFINE_TYPE (RejillaSplitDialog, rejilla_split_dialog, GTK_TYPE_DIALOG);

void
rejilla_split_dialog_set_uri (RejillaSplitDialog *self,
			      const gchar *uri,
                              const gchar *title,
                              const gchar *artist)
{
	...
	rejilla_song_control_set_uri (REJILLA_SONG_CONTROL (priv->player), uri);
	rejilla_song_control_set_info (REJILLA_SONG_CONTROL (priv->player), title, artist);
}

void
rejilla_split_dialog_set_boundaries (RejillaSplitDialog *self,
				     gint64 start,
				     gint64 end)
{
	RejillaSplitDialogPrivate *priv;
	...
	if (end - start < REJILLA_MIN_STREAM_LENGTH)
		return;

	priv->start = start;
	priv->end = end;

	rejilla_song_control_set_boundaries (REJILLA_SONG_CONTROL (priv->player),
	                                     priv->start,
	                                     priv->end);
}
...
rejilla_split_dialog_get_slices (RejillaSplitDialog *self)
{
	...
	do {
		...
		slice = g_new0 (RejillaAudioSlice, 1);
...
static gboolean
rejilla_split_dialog_size_error (RejillaSplitDialog *self)
{
	GtkWidget *message;
...
static void
rejilla_split_dialog_no_silence_message (RejillaSplitDialog *self)
{
...
static void
rejilla_split_dialog_metadata_finished_cb (RejillaMetadata *metadata,
					   GError *error,
					   RejillaSplitDialog *self)
{
	RejillaMetadataInfo info = { NULL, };
	RejillaSplitDialogPrivate *priv;
	...
	rejilla_metadata_get_result (metadata, &info, NULL);
	if (!info.silences) {
		rejilla_split_dialog_no_silence_message (self);
		return;
	}
	...
	for (iter = info.silences; iter; iter = iter->next) {
		...
		if (!silence->start)
			rejilla_split_dialog_cut (self, silence->end, TRUE);
		else if (silence->start != silence->end)
			rejilla_split_dialog_remove_range (self,
							   silence->start,
							   silence->end,
							   priv->end - priv->start);
		else
...
static gboolean
rejilla_split_dialog_clear_confirm_dialog (RejillaSplitDialog *self,
					   const gchar *primary,
...
static void
rejilla_split_dialog_cut_clicked_cb (GtkButton *button,
				     RejillaSplitDialog *self)
{
	RejillaSplitDialogPrivate *priv;
	GtkTreeModel *model;
	...
	if (page == 0) {
		...
		pos = rejilla_song_control_get_pos (REJILLA_SONG_CONTROL (priv->player));
		rejilla_split_dialog_cut (self, pos + priv->start, TRUE);
		return;
	}

	model = gtk_tree_view_get_model (GTK_TREE_VIEW (priv->tree));
	if (!rejilla_split_dialog_clear_confirm_dialog (self,
	...
	if (page == 1) {
		...
		sec *= 1000000000;
		if (sec < REJILLA_MIN_STREAM_LENGTH
		&& !rejilla_split_dialog_size_error (self))
			return;
		...
		for (start = sec; start < length; start += sec)
			rejilla_split_dialog_cut (self, start, FALSE);
	...
	if (page == 2) {
		...
		if (step < REJILLA_MIN_STREAM_LENGTH
		&& !rejilla_split_dialog_size_error (self))
			return;
	...
	priv->metadata = rejilla_metadata_new ();
	g_signal_connect (priv->metadata,
			  "completed",
			  G_CALLBACK (rejilla_split_dialog_metadata_finished_cb),
			  self);
	rejilla_metadata_get_info_async (priv->metadata,
					 rejilla_song_control_get_uri (REJILLA_SONG_CONTROL (priv->player)),
					 REJILLA_METADATA_FLAG_SILENCES);
...
static void
rejilla_split_dialog_reset_clicked_cb (GtkButton *button,
				       RejillaSplitDialog *self)
{
	RejillaSplitDialogPrivate *priv;
	GtkTreeModel *model;
...
static void
rejilla_split_dialog_combo_changed_cb (GtkComboBox *combo,
				       RejillaSplitDialog *self)
{
	RejillaSplitDialogPrivate *priv;
	guint page;
...
static void
rejilla_split_dialog_selection_changed_cb (GtkTreeSelection *selection,
					   RejillaSplitDialog *self)
{
	RejillaSplitDialogPrivate *priv;
	GList *selected;
...
static void
rejilla_split_dialog_row_inserted_cb (GtkTreeModel *model,
				      GtkTreePath *path,
				      GtkTreeIter *iter,
				      RejillaSplitDialog *self)
{
...
static void
rejilla_split_dialog_row_deleted_cb (GtkTreeModel *model,
				     GtkTreePath *path,
				     RejillaSplitDialog *self)
{
	RejillaSplitDialogPrivate *priv;
...
static void
rejilla_split_dialog_finalize (GObject *object)
{
	RejillaSplitDialogPrivate *priv;

	priv = REJILLA_SPLIT_DIALOG_PRIVATE (object);
	if (priv->metadata) {
		rejilla_metadata_cancel (priv->metadata);
		g_object_unref (priv->metadata);
		priv->metadata = NULL;
	}

	G_OBJECT_CLASS (rejilla_split_dialog_parent_class)->finalize (object);
}

static void
rejilla_split_dialog_class_init (RejillaSplitDialogClass *klass)
{
	GObjectClass* object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (RejillaSplitDialogPrivate));

	object_class->finalize = rejilla_split_dialog_finalize;
}

GtkWidget *
rejilla_split_dialog_new (void)
{
	return GTK_WIDGET (g_object_new (REJILLA_TYPE_SPLIT_DIALOG, NULL));
}
