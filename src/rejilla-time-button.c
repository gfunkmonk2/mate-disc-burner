...
typedef struct _RejillaTimeButtonPrivate RejillaTimeButtonPrivate;
struct _RejillaTimeButtonPrivate
{
	GtkWidget *hrs;
	GtkWidget *min;
	GtkWidget *sec;
...
#define REJILLA_TIME_BUTTON_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), REJILLA_TYPE_TIME_BUTTON, RejillaTimeButtonPrivate))
...
G_DEFINE_TYPE (RejillaTimeButton, rejilla_time_button, GTK_TYPE_HBOX);

static void
rejilla_time_button_hrs_changed (GtkSpinButton *button,
				 RejillaTimeButton *self)
{
	RejillaTimeButtonPrivate *priv;
	gint hrs, min;

	priv = REJILLA_TIME_BUTTON_PRIVATE (self);
...
static void
rejilla_time_button_min_changed (GtkSpinButton *button,
				 RejillaTimeButton *self)
{
	RejillaTimeButtonPrivate *priv;
	gint hrs, min, sec;
...
static void
rejilla_time_button_sec_changed (GtkSpinButton *button,
				 RejillaTimeButton *self)
{
	RejillaTimeButtonPrivate *priv;
...
static void
rejilla_time_button_frame_changed (GtkSpinButton *button,
				   RejillaTimeButton *self)
{
	g_signal_emit (self,
		       time_button_signals [VALUE_CHANGED_SIGNAL],
...
void
rejilla_time_button_set_show_frames (RejillaTimeButton *self,
				     gboolean show)
{
	RejillaTimeButtonPrivate *priv;
...
void
rejilla_time_button_set_max (RejillaTimeButton *self,
			     gint64 max)
{
	RejillaTimeButtonPrivate *priv;
	gint64 frames;
...
void
rejilla_time_button_set_value (RejillaTimeButton *self,
			       gint64 value)
{
	RejillaTimeButtonPrivate *priv;
	gint64 frames;
...
gint64
rejilla_time_button_get_value (RejillaTimeButton *self)
{
	RejillaTimeButtonPrivate *priv;
	gint64 value;
...
static void
rejilla_time_button_init (RejillaTimeButton *object)
{
	RejillaTimeButtonPrivate *priv;
	GtkWidget *label;

	priv = REJILLA_TIME_BUTTON_PRIVATE (object);
	...
	g_signal_connect (priv->hrs,
			  "value-changed",
			  G_CALLBACK (rejilla_time_button_hrs_changed),
			  object);
	g_signal_connect (priv->min,
			  "value-changed",
			  G_CALLBACK (rejilla_time_button_min_changed),
			  object);
	g_signal_connect (priv->sec,
			  "value-changed",
			  G_CALLBACK (rejilla_time_button_sec_changed),
			  object);
	g_signal_connect (priv->frame,
			  "value-changed",
			  G_CALLBACK (rejilla_time_button_frame_changed),
			  object);
}

static void
rejilla_time_button_finalize (GObject *object)
{
	G_OBJECT_CLASS (rejilla_time_button_parent_class)->finalize (object);
}

static void
rejilla_time_button_class_init (RejillaTimeButtonClass *klass)
{
	GObjectClass* object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (RejillaTimeButtonPrivate));

	object_class->finalize = rejilla_time_button_finalize;

	time_button_signals[VALUE_CHANGED_SIGNAL] =
		g_signal_new ("value-changed",
		              G_OBJECT_CLASS_TYPE (klass),
		              G_SIGNAL_RUN_FIRST | G_SIGNAL_NO_RECURSE,
		              G_STRUCT_OFFSET (RejillaTimeButtonClass, value_changed),
		              NULL, NULL,
...
{
	return g_object_new (REJILLA_TYPE_TIME_BUTTON, NULL);
}
