...
G_DEFINE_TYPE (RejillaSongProps, rejilla_song_props, GTK_TYPE_DIALOG);

struct RejillaSongPropsPrivate {
       	GtkWidget *title;
	GtkWidget *artist;
	GtkWidget *composer;
	GtkWidget *isrc;
...
static void
rejilla_song_props_update_length (RejillaSongProps *self)
{
	gchar *length_str;
	gint64 start;
	gint64 end;
	...
	end = rejilla_time_button_get_value (REJILLA_TIME_BUTTON (self->priv->end));
	start = rejilla_time_button_get_value (REJILLA_TIME_BUTTON (self->priv->start));
	gap = gtk_spin_button_get_value (GTK_SPIN_BUTTON (self->priv->gap)) * GST_SECOND;

	length_str = rejilla_units_get_time_string (REJILLA_STREAM_LENGTH (start, end + gap), TRUE, FALSE);
	gtk_label_set_markup (GTK_LABEL (self->priv->length), length_str);
	g_free (length_str);
}

static void
rejilla_song_props_end_changed_cb (RejillaTimeButton *button,
				   RejillaSongProps *self)
{
	gint64 end;

	end = rejilla_time_button_get_value (REJILLA_TIME_BUTTON (self->priv->end));
	rejilla_time_button_set_max (REJILLA_TIME_BUTTON (self->priv->start), end - 1);
	rejilla_song_props_update_length (self);
}

static void
rejilla_song_props_start_changed_cb (RejillaTimeButton *button,
				     RejillaSongProps *self)
{
	rejilla_song_props_update_length (self);
}
...
static void
rejilla_song_props_gap_changed_cb (GtkSpinButton *button,
				   RejillaSongProps *self)
{
	gchar *length_str;
	gint64 start;
...
static void
rejilla_song_props_title_set (GtkEntry *entry,
			      RejillaSongProps *self)
{
...
static void
rejilla_song_props_artist_set (GtkEntry *entry,
			       RejillaSongProps *self)
{
...
static void
rejilla_song_props_composer_set (GtkEntry *entry,
				 RejillaSongProps *self)
{
...
static void
rejilla_song_props_init (RejillaSongProps *obj)
{
	gchar *title_str;
	GtkWidget *label;
	GtkWidget *table;
	...
	obj->priv = g_new0 (RejillaSongPropsPrivate, 1);
	...
	frame = rejilla_utils_pack_properties ("", table, NULL);
	...
	obj->priv->label = rejilla_utils_properties_get_label (frame);
	...
	g_signal_connect (obj->priv->title,
			  "changed",
			  G_CALLBACK (rejilla_song_props_title_set),
			  obj);
	...
	g_signal_connect (obj->priv->artist,
			  "changed",
			  G_CALLBACK (rejilla_song_props_artist_set),
			  obj);
	...
	g_signal_connect (obj->priv->composer,
			  "changed",
			  G_CALLBACK (rejilla_song_props_composer_set),
			  obj);
	...
	frame = rejilla_utils_pack_properties (title_str,
					       table,
					       NULL);
	g_free (title_str);
	...
	g_signal_connect (obj->priv->end,
			  "value-changed",
			  G_CALLBACK (rejilla_song_props_end_changed_cb),
			  obj);
	g_signal_connect (obj->priv->start,
			  "value-changed",
			  G_CALLBACK (rejilla_song_props_start_changed_cb),
			  obj);
	g_signal_connect (obj->priv->gap,
			  "value-changed",
			  G_CALLBACK (rejilla_song_props_gap_changed_cb),
			  obj);
...
static gchar *
rejilla_song_props_get_entry_value (GtkEntry *entry)
{
	const gchar *text;
				   ...
				   gint64 *gap)
{
	if (artist) {
		if (self->priv->artist_set)
			*artist = rejilla_song_props_get_entry_value (GTK_ENTRY (self->priv->artist));
		else
			*artist = NULL;
	}
	...
	if (title) {
		if (self->priv->title_set)
			*title = rejilla_song_props_get_entry_value (GTK_ENTRY (self->priv->title));
		else
			*title = NULL;
	}
	...
	if (composer) {
		if (self->priv->composer_set)
			*composer = rejilla_song_props_get_entry_value (GTK_ENTRY (self->priv->composer));
		else
			*composer = NULL;
	}
	...
	if (isrc) {
		...
		string = rejilla_song_props_get_entry_value (GTK_ENTRY (self->priv->isrc));
		if (string)
				   ...
				   gint64 gap)
{
	...
	if (track_num >= 0) {
		...
		rejilla_time_button_set_show_frames (REJILLA_TIME_BUTTON (self->priv->start), FALSE);
		rejilla_time_button_set_show_frames (REJILLA_TIME_BUTTON (self->priv->end), FALSE);
	...
	rejilla_time_button_set_max (REJILLA_TIME_BUTTON (self->priv->start), end - 1);
	rejilla_time_button_set_value (REJILLA_TIME_BUTTON (self->priv->start), start);

	g_signal_handlers_block_by_func (self->priv->end,
					 rejilla_song_props_end_changed_cb,
					 self);
	rejilla_time_button_set_max (REJILLA_TIME_BUTTON (self->priv->end), start + length);
	rejilla_time_button_set_value (REJILLA_TIME_BUTTON (self->priv->end), end);
...
static void
rejilla_song_props_finalize (GObject *object)
{
	RejillaSongProps *cobj;

	cobj = REJILLA_SONG_PROPS(object);
...
static void
rejilla_song_props_class_init (RejillaSongPropsClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);

	parent_class = g_type_class_peek_parent(klass);
	object_class->finalize = rejilla_song_props_finalize;
}

GtkWidget *
rejilla_song_props_new ()
{
	RejillaSongProps *obj;
	
	obj = REJILLA_SONG_PROPS (g_object_new (REJILLA_TYPE_SONG_PROPS, NULL));
