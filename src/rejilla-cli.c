...
RejillaCLI cmd_line_options;

static gboolean
rejilla_cli_copy_project (const gchar *option_name,
                          const gchar *value,
                          gpointer data,
                          GError **error);
static gboolean
rejilla_cli_image_project (const gchar *option_name,
                           const gchar *value,
                           gpointer data,
                           GError **error);

static gboolean
rejilla_cli_fake_device (const gchar *option_name,
                         const gchar *value,
                         gpointer data,
                         GError **error);

static gboolean
rejilla_cli_burning_device (const gchar *option_name,
                            const gchar *value,
                            gpointer data,
                            GError **error);
...
const GOptionEntry prog_options [] = {
	...
	{ "device", 0, G_OPTION_FLAG_FILENAME, G_OPTION_ARG_CALLBACK, rejilla_cli_burning_device,
	  N_("Set the drive to be used for burning"),
	  N_("DEVICE PATH") },

	{ "image-file", 0, G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK, rejilla_cli_fake_device,
	...
	{ "copy", 'c', G_OPTION_FLAG_OPTIONAL_ARG|G_OPTION_FLAG_FILENAME, G_OPTION_ARG_CALLBACK, rejilla_cli_copy_project,
	  N_("Copy a disc"),
	  N_("PATH TO DEVICE") },
	...
	{ "image", 'i', G_OPTION_FLAG_OPTIONAL_ARG|G_OPTION_FLAG_FILENAME, G_OPTION_ARG_CALLBACK, rejilla_cli_image_project,
	 N_("URI of an image file to burn (autodetected)"),
          N_("PATH TO IMAGE") },

    	{ "empty", 'e', 0, G_OPTION_ARG_NONE, &cmd_line_options.empty_project,
         N_("Force Rejilla to display the project selection page"),
          NULL },
	 ...
	 * rejilla will be set to be always on top of the window identified by
	 * xid. In other words, the window with the given xid will become rejilla
	 * parent as if rejilla was a dialog for the parent application */
                         ...
                         GError **error)
{
	...
	/* Wait for the librejilla-media to be ready */
	monitor = rejilla_medium_monitor_get_default ();
	while (rejilla_medium_monitor_is_probing (monitor))
		sleep (1);

	list = rejilla_medium_monitor_get_drives (monitor, REJILLA_DRIVE_TYPE_FILE);
	if (!list)
		return FALSE;
                            ...
                            GError **error)
{
	...
	burner = rejilla_medium_monitor_get_drive (monitor, value);
	g_object_unref (monitor);

	if (burner) {
		if (!rejilla_drive_can_write (burner)) {
			gchar *string;

			/* Translators: %s is the path of drive */
			string = g_strdup_printf (_("\"%s\" cannot write."), value);
			rejilla_utils_message_dialog (NULL,
...
rejilla_cli_apply_options (RejillaApp *app)
{
	...
	if (cmd_line_options.parent_window)
		rejilla_app_set_parent (app, cmd_line_options.parent_window);

    	if (cmd_line_options.empty_project) {
	    	rejilla_app_create_mainwin (app);
		manager = rejilla_app_get_project_manager (app);
		rejilla_project_manager_empty (REJILLA_PROJECT_MANAGER (manager));
		rejilla_app_run_mainwin (app);
		return;
	}
	...
	if (nb > 1) {
		...
		res = rejilla_app_open_project (app,
		                                cmd_line_options.burner,
		...
		rejilla_app_burn_uri (app, cmd_line_options.burner, cmd_line_options.burn_immediately != 0);
		return;
	}
	else if (cmd_line_options.disc_blank) {
		rejilla_app_blank (app, cmd_line_options.burner, cmd_line_options.burn_immediately != 0);
		return;
	}
	else if (cmd_line_options.disc_check) {
		rejilla_app_check (app, cmd_line_options.burner, cmd_line_options.burn_immediately != 0);
		return;
	}
	else if (cmd_line_options.audio_project) {
		rejilla_app_stream (app, cmd_line_options.burner, cmd_line_options.files, FALSE, cmd_line_options.burn_immediately != 0);
		if (cmd_line_options.burn_immediately)
			return;
	}
	else if (cmd_line_options.data_project) {
		rejilla_app_data (app, cmd_line_options.burner, cmd_line_options.files, cmd_line_options.burn_immediately != 0);
		if (cmd_line_options.burn_immediately)
			return;
	}
	else if (cmd_line_options.video_project) {
		rejilla_app_stream (app, cmd_line_options.burner, cmd_line_options.files, TRUE, cmd_line_options.burn_immediately != 0);
		if (cmd_line_options.burn_immediately)
			return;
	}
		...
		if (g_strv_length (cmd_line_options.files) == 1) {
			...
			result = rejilla_app_open_uri_drive_detection (app,
			                                               cmd_line_options.burner,
			...
			result = rejilla_app_open_uri (app, cmd_line_options.files [0], FALSE);
			if (!result)
				rejilla_app_data (app, cmd_line_options.burner, cmd_line_options.files, cmd_line_options.burn_immediately != 0);
		}
		else
