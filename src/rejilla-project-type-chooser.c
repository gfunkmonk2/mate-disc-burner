...
G_DEFINE_TYPE (RejillaProjectTypeChooser, rejilla_project_type_chooser, GTK_TYPE_EVENT_BOX);
...
typedef enum {
	...
	CHOSEN_SIGNAL,
	LAST_SIGNAL
} RejillaProjectTypeChooserSignalType;
static guint rejilla_project_type_chooser_signals [LAST_SIGNAL] = { 0 };

enum {
	REJILLA_PROJECT_TYPE_CHOOSER_ICON_COL,
	REJILLA_PROJECT_TYPE_CHOOSER_TEXT_COL,
	REJILLA_PROJECT_TYPE_CHOOSER_ID_COL,
	REJILLA_PROJECT_TYPE_CHOOSER_NB_COL
};
...
struct RejillaProjectTypeChooserPrivate {
...
static void
rejilla_project_type_chooser_button_clicked (GtkButton *button,
					     RejillaProjectTypeChooser *chooser)
{
	RejillaProjectType type;

	type = GPOINTER_TO_INT (g_object_get_data (G_OBJECT (button), ID_KEY));
	g_signal_emit (chooser,
		       rejilla_project_type_chooser_signals [CHOSEN_SIGNAL],
		       0,
		       type);
}

static GtkWidget *
rejilla_project_type_chooser_new_item (RejillaProjectTypeChooser *chooser,
				       ItemDescription *description)
{
	...
	g_signal_connect (eventbox,
			  "clicked",
			  G_CALLBACK (rejilla_project_type_chooser_button_clicked),
			  chooser);
...
static void
rejilla_project_type_chooser_recent_clicked_cb (GtkButton *button,
						RejillaProjectTypeChooser *self)
{
	const gchar *uri;

	uri = g_object_get_data (G_OBJECT (button), "RejillaButtonURI");
	g_signal_emit (self,
		       rejilla_project_type_chooser_signals [RECENT_CLICKED_SIGNAL],
		       0,
		       uri);
}

static void
rejilla_project_type_chooser_last_unsaved_clicked_cb (GtkButton *button,
						      RejillaProjectTypeChooser *self)
{
	const gchar *uri;
	gchar *path;
	...
	g_signal_emit (self,
		       rejilla_project_type_chooser_signals [LAST_SAVED_CLICKED_SIGNAL],
		       0,
		       path);
...
static gint
rejilla_project_type_chooser_sort_recent (gconstpointer a, gconstpointer b)
{
...
static void
rejilla_project_type_chooser_build_recent (RejillaProjectTypeChooser *self,
					   GtkRecentManager *recent)
{
	...
	for (iter = recents; iter; iter = iter->next) {
		...
		list = g_list_insert_sorted (list,
					     info,
					     rejilla_project_type_chooser_sort_recent);
	...
	filename = g_build_filename (g_get_user_config_dir (),
				     "rejilla",
				     REJILLA_SESSION_TMP_PROJECT_PATH,
				     NULL);
	if (g_file_test (filename, G_FILE_TEST_EXISTS)) {
		...
		image = gtk_image_new_from_icon_name ("rejilla", GTK_ICON_SIZE_BUTTON);
		...
		g_signal_connect (link,
				  "clicked",
				  G_CALLBACK (rejilla_project_type_chooser_last_unsaved_clicked_cb),
				  self);
	...
	for (iter = list; iter; iter = iter->next) {
		...
		g_signal_connect (link,
				  "clicked",
				  G_CALLBACK (rejilla_project_type_chooser_recent_clicked_cb),
				  self);
		gtk_widget_show (link);
...
static void
rejilla_project_type_chooser_recent_changed_cb (GtkRecentManager *recent,
						RejillaProjectTypeChooser *self)
{
	gtk_container_foreach (GTK_CONTAINER (self->priv->recent_box),
			       (GtkCallback) gtk_widget_destroy,
			       NULL);
	rejilla_project_type_chooser_build_recent (self, recent);
}

static void
rejilla_project_type_chooser_init (RejillaProjectTypeChooser *obj)
{
	GtkRecentManager *recent;
	...
	obj->priv = g_new0 (RejillaProjectTypeChooserPrivate, 1);

	obj->priv->background = gdk_pixbuf_new_from_file (REJILLA_DATADIR "/logo.png", &error);
	if (error) {
	...
	for (i = 0; i < nb_items; i ++) {
		widget = rejilla_project_type_chooser_new_item (obj, items + i);
	...
	rejilla_project_type_chooser_build_recent (obj, recent);

	g_signal_connect (recent,
			  "changed",
			  G_CALLBACK (rejilla_project_type_chooser_recent_changed_cb),
			  obj);
}

/* Cut and Pasted from Gtk+ gtkeventbox.c but modified to display back image */
static gboolean
rejilla_project_type_expose_event (GtkWidget *widget, GdkEventExpose *event)
{
	RejillaProjectTypeChooser *chooser;

	chooser = REJILLA_PROJECT_TYPE_CHOOSER (widget);
...
static void
rejilla_project_type_chooser_finalize (GObject *object)
{
	RejillaProjectTypeChooser *cobj;

	cobj = REJILLA_PROJECT_TYPE_CHOOSER (object);
...
static void
rejilla_project_type_chooser_class_init (RejillaProjectTypeChooserClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);
	GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

	parent_class = g_type_class_peek_parent (klass);
	object_class->finalize = rejilla_project_type_chooser_finalize;
	widget_class->expose_event = rejilla_project_type_expose_event;

	rejilla_project_type_chooser_signals[CHOSEN_SIGNAL] =
	    g_signal_new ("chosen", G_OBJECT_CLASS_TYPE (object_class),
			  G_SIGNAL_ACTION | G_SIGNAL_RUN_FIRST,
			  G_STRUCT_OFFSET (RejillaProjectTypeChooserClass, chosen),
			  NULL, NULL,
	...
	rejilla_project_type_chooser_signals[RECENT_CLICKED_SIGNAL] =
	    g_signal_new ("recent_clicked", G_OBJECT_CLASS_TYPE (object_class),
			  G_SIGNAL_ACTION | G_SIGNAL_RUN_FIRST,
			  G_STRUCT_OFFSET (RejillaProjectTypeChooserClass, recent_clicked),
			  NULL, NULL,
	...
	rejilla_project_type_chooser_signals[LAST_SAVED_CLICKED_SIGNAL] =
	    g_signal_new ("last_saved", G_OBJECT_CLASS_TYPE (object_class),
			  G_SIGNAL_ACTION | G_SIGNAL_RUN_FIRST,
			  G_STRUCT_OFFSET (RejillaProjectTypeChooserClass, last_saved_clicked),
			  NULL, NULL,
			  g_cclosure_marshal_VOID__STRING,
			  G_TYPE_NONE,
			  1,
...
GtkWidget *
rejilla_project_type_chooser_new ()
{
	RejillaProjectTypeChooser *obj;

	obj = REJILLA_PROJECT_TYPE_CHOOSER (g_object_new (REJILLA_TYPE_PROJECT_TYPE_CHOOSER,
					    NULL));
